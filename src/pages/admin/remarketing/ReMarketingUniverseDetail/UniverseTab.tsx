          ...
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                {(() => {
                  const unenrichedCount = universeDeals?.filter((d) => !(d.listing as { enriched_at?: string | null } | null)?.enriched_at).length || 0;
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      const listingIds = universeDeals.filter((d) => (d.listing as { id?: string } | null)?.id).map((d) => (d.listing as { id: string }).id);
                ...
                {(dealQueueProgress.isEnriching || dealQueueProgress.isPaused) ? (
                  ...
                  (() => {
                    const unenrichedDeals = universeDeals?.filter((d) => {
                      const listing = d.listing as { id?: string; enriched_at?: string | null } | null;
