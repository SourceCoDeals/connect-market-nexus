...
export function useMatchingData(listingId: string | undefined) {
  ...
  // Custom scoring instructions state — persisted to localStorage keyed by listingId
  const [customInstructions, setCustomInstructions] = useState(() => {
    if (!listingId) return '';
    try {
      return localStorage.getItem(`scoring-instructions-${listingId}`) || '';
    } catch {
      return '';
    }
  });
  ...
  // Initialize custom instructions from saved data (database takes precedence)
  useEffect(() => {
  ...
  // Persist custom instructions to localStorage on change
  useEffect(() => {
    if (!listingId) return;
    try {
      if (customInstructions) {
        localStorage.setItem(`scoring-instructions-${listingId}`, customInstructions);
      } else {
        localStorage.removeItem(`scoring-instructions-${listingId}`);
      }
    } catch {
      // localStorage unavailable — ignored
    }
  }, [customInstructions, listingId]);
