...
/** Format a compact valuation range like "$11.8–13.7M" — single unit suffix, en-dash. */
export function formatCompactRange(low: number | null, high: number | null): string | null {
  if (low == null || high == null) return null;
  const unit = (n: number) => {
    if (Math.abs(n) >= 1e9) return { divisor: 1e9, suffix: 'B' };
    if (Math.abs(n) >= 1e6) return { divisor: 1e6, suffix: 'M' };
    if (Math.abs(n) >= 1e3) return { divisor: 1e3, suffix: 'K' };
    return { divisor: 1, suffix: '' };
  };
  const hi = unit(high);
  const lowDivisor = hi.divisor;
  const fmt = (n: number) => {
    const v = n / lowDivisor;
    return v >= 100 ? Math.round(v).toString() : v.toFixed(1).replace(/\.0$/, '');
  };
  return `$${fmt(low)}\u2013${fmt(high)}${hi.suffix}`;
}

export type SublineKind = 'website' | 'email' | 'none';

export interface DisplayIdentifier {
  name: string;
  subline: string | null;
  sublineHref: string | null;
  sublineKind: SublineKind;
}

/** Humanize an email's local-part: "john.doe@x.com" → "John Doe". Returns null if low quality. */
function humanizeEmailLocalPart(email: string): string | null {
  const local = email.split('@')[0];
  if (!local) return null;
  // Skip if it's mostly numbers or junk
...
/**
 * Resolve the best display identifier for a lead — name + subline (website OR email).
 * Falls back to email-based identity when website is missing.
 */
export function extractDisplayIdentifier(lead: ValuationLead): DisplayIdentifier {
  const name = extractBusinessName(lead);
  ...
  if (website) {
    return {
      name,
      subline: website,
      sublineHref: `https://${website}`,
      sublineKind: 'website',
    };
  }

  if (lead.email) {
    // If the resolved business name is still a placeholder (e.g. "General Calculator #213"),
    // try to humanize the email's local-part.
    const looksPlaceholder =
      isPlaceholderBusinessName(name) ||
      /^(general|hvac|auto|collision|specialty|mechanical)\s+calculator\b/i.test(name) ||
      name === '\u2014';
    const finalName = looksPlaceholder
      ? humanizeEmailLocalPart(lead.email) || cleanFullName(lead.full_name) || lead.email
      : name;

    return {
      name: finalName,
      subline: lead.email,
      sublineHref: `mailto:${lead.email}`,
      sublineKind: 'email',
    };
  }
