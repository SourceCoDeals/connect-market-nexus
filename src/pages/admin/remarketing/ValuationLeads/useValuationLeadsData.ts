...
export function useValuationLeadsData() {
      ...
      if (listingIds.length > 0) {
        ...
        try {
          const result = await startOrQueueMajorOp({
            operationType: "deal_enrichment", totalItems: listingIds.length,
            description: `Push & enrich ${listingIds.length} valuation leads`,
            userId: user?.id || "", contextJson: { source: "valuation_leads_push_enrich" },
          });
          activityItem = result.item;
        } catch { /* Non-blocking */ }

        const now = new Date().toISOString();
        const rows = listingIds.map((id) => ({ listing_id: id, status: "pending" as const, attempts: 0, queued_at: now }));
        const CHUNK = 500;
        ...
        try {
          const { data: result, error: resultError } = await supabase.functions.invoke("process-enrichment-queue", { body: { source: "valuation_leads_push_enrich" } });
  ...
  const handleReEnrich = useCallback(
    async (leadIds: string[]) => {
      ...
      try {
        const result = await startOrQueueMajorOp({
          operationType: "deal_enrichment", totalItems: leadsToProcess.length,
          description: `Re-enriching ${leadsToProcess.length} valuation leads`,
          userId: user?.id || "", contextJson: { source: "valuation_leads_re_enrich" },
        });
        activityItem = result.item;
      } catch { /* Non-blocking */ }
      ...
      const rows = leadsToProcess
        ...
        .map((l) => ({
          ...
          status: "pending" as const, attempts: 0, queued_at: now,
          force: true, completed_at: null, last_error: null, started_at: null,
        }));
  ...
  const handleArchive = useCallback(async (leadIds: string[]) => {
    ...
    const { error } = await supabase
      ...
      .in("id", leadIds);
    if (error) { sonnerToast.error("Failed to archive leads"); return; }
    ...
    async (mode: "unenriched" | "all") => {
          ...
          await Promise.all(
            batch.map(async (lead) => {
              try {
                const dealIdentifier = `vlead_${lead.id.slice(0, 8)}`;
                const { data: existing, error: existingError } = await supabase
                  .from("listings").select("id").eq("deal_identifier", dealIdentifier).maybeSingle();
                if (existingError) throw existingError;
                let listingId: string;
                if (existing?.id) { listingId = existing.id; }
                else {
                  const { data: listing, error: insertError } = await supabase
                    .from("listings").insert(buildListingFromLead(lead, false)).select("id").single();
                  if (insertError || !listing) return;
                  listingId = listing.id;
                }
                await supabase.from("valuation_leads").update({ pushed_listing_id: listingId } as never).eq("id", lead.id);
                lead.pushed_listing_id = listingId;
              } catch { /* Non-blocking per-lead */ }
            })
          );
        }
      }
      ...
      try {
        const result = await startOrQueueMajorOp({
          ...
          userId: user?.id || "", contextJson: { source: "valuation_leads_bulk" },
        });
        activityItem = result.item;
      } catch { /* Non-blocking */ }
      ...
      const rows = targets
        ...
        .map((l) => ({
          listing_id: l.pushed_listing_id!,
          status: "pending" as const, attempts: 0, queued_at: now,
          force: mode === "all", completed_at: null, last_error: null, started_at: null,
        }));
      ...
      try {
        const { data: result, error: resultError } = await supabase.functions.invoke("process-enrichment-queue", { body: { source: "valuation_leads_bulk" } });
        if (resultError) throw resultError;
        if (result?.synced > 0 || result?.processed > 0) {
          const totalDone = (result?.synced || 0) + (result?.processed || 0);
          if (activityItem) updateProgress.mutate({ id: activityItem.id, completedItems: totalDone });
  ...
  const handleScoreLeads = useCallback(
    async (mode: "unscored" | "all") => {
      const targets = mode === "unscored"
        ...
        : filteredLeads;
      if (!targets.length) { sonnerToast.info("No leads to score"); return; }
      setIsScoring(true);
      sonnerToast.info(`Scoring ${targets.length} leads...`);
      try {
        const { data, error } = await supabase.functions.invoke("calculate-valuation-lead-score", { body: { mode } });
        if (error) throw error;
  ...
  const handleAssignOwner = useCallback(async (lead: ValuationLead, ownerId: string | null) => {
    const { error } = await supabase.from("valuation_leads").update({ deal_owner_id: ownerId }).eq("id", lead.id);
    if (error) { sonnerToast.error("Failed to update owner"); return; }
    if (lead.pushed_listing_id) {
      await supabase.from("listings").update({ deal_owner_id: ownerId }).eq("id", lead.pushed_listing_id);
    }
  ...
  const kpiStats = useMemo(() => {
    ...
    const avgScore = filteredLeads.length > 0
      ? Math.round(filteredLeads.reduce((sum, l) => sum + (l.lead_score ?? 0), 0) / filteredLeads.length) : 0;
