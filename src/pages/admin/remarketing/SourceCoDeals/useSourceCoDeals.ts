...
import type { SourceCoDeal, SortColumn, SortDirection } from './types';
      ...
      while (hasMore) {
        const { data, error } = await supabase
          ...
          .select(
            ...
            salesforce_account_id, sf_remarketing_reason, sf_primary_client_account,
            sf_interest_in_selling, sf_seller_interest_score, sf_note_summary,
            sf_one_pager, sf_lead_memo, sf_nda,
            deal_owner:profiles!listings_deal_owner_id_fkey(id, first_name, last_name, email)
          `,
          )
          .eq('deal_source', 'salesforce_remarketing')
    ...
    items.sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'priority':
          ...
          break;
        case 'sf_interest':
          valA = (a.sf_interest_in_selling || '').toLowerCase();
          valB = (b.sf_interest_in_selling || '').toLowerCase();
          break;
        default:
          return 0;
      }
    ...
    async (mode: 'unenriched' | 'all') => {
      ...
      try {
        const result = await startOrQueueMajorOp({
          ...
          userId: user?.id || '',
          contextJson: { source: 'salesforce_remarketing' },
        });
