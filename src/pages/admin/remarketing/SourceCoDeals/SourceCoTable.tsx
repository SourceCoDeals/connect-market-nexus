...
import {
  Building2, ArrowUpDown, CheckCircle2,
...
export function SourceCoTable({
  ...
  handleAssignOwner, adminProfiles, onMarkNotFit,
}: SourceCoTableProps) {
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="max-w-[200px]">Remarketing Reason</TableHead>
                <TableHead><SortHeader column="industry">Industry</SortHeader></TableHead>
                <TableHead><SortHeader column="sf_interest">Seller Interest</SortHeader></TableHead>
              ...
              {paginatedDeals.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={14} className="text-center py-12 text-muted-foreground">
                    ...
                    <p className="text-sm mt-1">
                      Deals will appear here automatically when Remarketing is enabled in Salesforce.
                    </p>
                  </TableCell>
                </TableRow>
              ) : (
                paginatedDeals.map((deal, index) => (
                  <TableRow
                    ...
                    onClick={() => navigate(`/admin/remarketing/leads/sourceco/${deal.id}`, { state: { from: "/admin/remarketing/leads/sourceco" } })}
                  >
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        ...
                        {deal.sf_primary_client_account && (
                          <span className="text-[10px] text-muted-foreground/70 truncate max-w-[220px]">
                            via {deal.sf_primary_client_account}
                          </span>
                        )}
                      </div>
                    </TableCell>
                    <TableCell className="max-w-[200px]">
                      <span className="text-xs text-muted-foreground line-clamp-3">
                        {deal.sf_remarketing_reason || deal.description || "\u2014"}
                      </span>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {deal.sf_interest_in_selling ? (
                        <Badge variant="outline" className={cn(
                          "text-[10px]",
                          deal.sf_interest_in_selling === 'Yes' && "bg-green-50 text-green-700 border-green-200",
                          deal.sf_interest_in_selling === 'Unsure' && "bg-yellow-50 text-yellow-700 border-yellow-200",
                          deal.sf_interest_in_selling === 'No' && "bg-red-50 text-red-700 border-red-200",
                        )}>
                          {deal.sf_interest_in_selling}
                        </Badge>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onClick={() => navigate(`/admin/remarketing/leads/sourceco/${deal.id}`, { state: { from: "/admin/remarketing/leads/sourceco" } })}>
        ...
        <DropdownMenuItem
          onClick={async () => {
            ...
            else { sonnerToast.success(newValue ? "Marked as priority" : "Priority removed"); queryClient.invalidateQueries({ queryKey: ["remarketing", "sourceco-deals"] }); }
          }}
        ...
        <DropdownMenuItem
          onClick={async () => {
            ...
            if (!error) { sonnerToast.success(newVal ? "Flagged: Needs Buyer Universe" : "Flag removed"); queryClient.invalidateQueries({ queryKey: ["remarketing", "sourceco-deals"] }); }
          }}
        ...
        <DropdownMenuItem
          onClick={async () => {
            ...
            if (!error) { sonnerToast.success(newVal ? "Flagged: Need to Contact Owner" : "Flag removed"); queryClient.invalidateQueries({ queryKey: ["remarketing", "sourceco-deals"] }); }
          }}
