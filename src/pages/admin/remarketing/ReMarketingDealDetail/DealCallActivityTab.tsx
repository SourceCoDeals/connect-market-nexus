        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {activity.call_duration_seconds ? (
            <span className="text-xs text-muted-foreground flex items-center gap-1">
              ...
              {activity.talk_time_seconds
                ? ` (talk: ${formatDuration(activity.talk_time_seconds)})`
                : ''}
            </span>
          ) : null}
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {activity.call_started_at && (
            <>
              ...
              <span className="text-muted-foreground/50">·</span>
              <span>
                {formatDistanceToNow(new Date(activity.call_started_at), { addSuffix: true })}
              </span>
            </>
          )}
        </div>
...
export function DealCallActivityTab({ listingId }: DealCallActivityTabProps) {
  ...
  // Compute quick stats
  const totalCalls = activities.filter(
    (a) => a.activity_type === 'call_completed' || a.activity_type === 'call_attempt',
  ).length;
  const connected = activities.filter((a) => a.call_connected).length;
  const totalTalkTime = activities.reduce(
    (sum, a) => sum + (a.talk_time_seconds || a.call_duration_seconds || 0),
    0,
  );
