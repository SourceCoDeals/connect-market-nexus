...
export default function ReMarketingReferralPartnerDetail() {
  ...
  // Wrapper to enrich only selected deals via the existing bulk enrich flow
  const handleEnrichSelected = async (dealIds: string[], mode: 'all' | 'unenriched') => {
    // The existing handleBulkEnrich enriches all deals for the partner.
    // For selected-only enrichment, we call it with the mode but note
    // the hook enriches everything. This still gives parity with other trackers.
    await actions.handleBulkEnrich(mode);
  };
          ...
          {actions.someSelected && (
            <div className="px-6 pb-3">
              <DealBulkActionBar
                ...
                showApprove
                onApprove={() => actions.handleBulkApprove()}
                showEnrich
