...
export function useExtractionHandlers(
  ...
  const handleExtractAll = async () => {
    ...
    for (let i = 0; i < transcripts.length; i++) {
      try {
        ...
        })) as { insights?: { buyer?: Record<string, unknown> } } | undefined;
        successCount++;
        results.push({
  ...
  const handleSingleExtractWithSummary = async (transcriptId: string) => {
    try {
      ...
      const data = (await extractTranscriptMutation.mutateAsync({ transcriptId })) as
        | { insights?: { buyer?: Record<string, unknown> } }
        | undefined;
      setExtractionSummary({
        open: true,
        results: [
      ...
      setExtractionSummary({
        ...
        results: [
          {
            fileName: transcript?.file_name || 'Transcript',
            error: e instanceof Error ? e.message : 'Failed',
          },
        ],
        totalCount: 1,
