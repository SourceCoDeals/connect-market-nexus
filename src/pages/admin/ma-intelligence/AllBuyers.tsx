    ...
    try {
      ...
      const [trackersRes, buyersRes, countRes] = await Promise.all([
        ...
        supabase
          ...
          .select(
            'id, company_name, pe_firm_name, platform_company_name, platform_website, thesis_summary, industry_vertical, industry_tracker_id, tracker_id, has_fee_agreement, fee_agreement_status',
          )
          .order('company_name')
          .range(from, to),
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              {uniqueFirms} PE firm{uniqueFirms !== 1 ? 's' : ''} · {totalCount} total platform
              {totalCount !== 1 ? 's' : ''}
              {totalPages > 1 && (
                <span>
                  {' '}
        ...
        {sortedBuyers.length === 0 ? (
          <div className="bg-card rounded-lg border p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {search ? 'Try a different search term' : 'Add buyers to a universe to get started.'}
            </p>
          </div>
        ) : (
          <>
                    ...
                    return (
                      <TableRow key={buyer.id} className="group">
                        <TableCell className="font-medium">
                          <Link
                            to={`/admin/ma-intelligence/buyers/${buyer.id}`}
                        ...
                        <TableCell className="text-center">
                          {(buyer.has_fee_agreement || buyer.fee_agreement_status === 'Signed') && (
                            <Badge
                              variant="outline"
                              className="text-xs flex items-center gap-1 bg-primary/10 border-primary/20 w-fit mx-auto"
                            >
