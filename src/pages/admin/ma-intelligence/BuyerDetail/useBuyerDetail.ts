...
export function useBuyerDetail() {
  ...
  const loadBuyer = useCallback(async () => {
    if (!id) return;
    try {
      const { data, error } = await supabase.from("remarketing_buyers").select("*").eq("id", id).single();
  ...
  useEffect(() => { if (id) loadBuyer(); }, [id, loadBuyer]);
  ...
  const handleArchive = async () => {
    if (!buyer || !confirm("Are you sure you want to archive this buyer?")) return;
    try {
      const { error } = await supabase.from("remarketing_buyers").update({ archived: true }).eq("id", buyer.id);
      if (error) throw error;
  ...
  const handleDelete = async () => {
    if (!buyer || !confirm("Are you sure you want to delete this buyer? This action cannot be undone.")) return;
    try {
      const { error } = await supabase.from("remarketing_buyers").delete().eq("id", buyer.id);
      if (error) throw error;
  ...
  const handleApproveForDeal = async () => {
    ...
    try {
      const { data: existingScore, error: existingScoreError } = await supabase.from("buyer_deal_scores").select("id").eq("buyer_id", buyer.id).eq("deal_id", dealId).single();
      if (existingScoreError) throw existingScoreError;
      if (existingScore) {
        const { error } = await supabase.from("buyer_deal_scores").update({ selected_for_outreach: true }).eq("id", existingScore.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("buyer_deal_scores").insert({ buyer_id: buyer.id, deal_id: dealId, selected_for_outreach: true, scored_at: new Date().toISOString() });
        if (error) throw error;
      }
  ...
  const handleSaveSection = async (_section: string) => {
    ...
    try {
      ...
      for (const key of schemaFields) {
        if (key in formData) updateData[key] = (formData as Record<string, unknown>)[key];
      }
      const { error } = await supabase.from("remarketing_buyers").update(updateData).eq("id", buyer.id);
      if (error) throw error;
  ...
  const handleCancelEdit = () => { setEditingSection(null); setFormData(buyer || {}); };

  const handleSaveNotes = async (notes: string) => {
    if (!buyer) return;
    const { error } = await supabase.from('remarketing_buyers').update({ notes }).eq('id', buyer.id);
  ...
  const handleAnalyzeNotes = async (notes: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('analyze-buyer-notes', { body: { buyerId: buyer.id, notesText: notes } });
      if (error) throw error;
      if (data?.success) {
  ...
  return {
    id, dealId, buyer, isLoading, activeTab, setActiveTab,
