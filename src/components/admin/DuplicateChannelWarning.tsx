...
export const DuplicateChannelWarning = ({
  sourceMetadata,
  className,
}: DuplicateChannelWarningProps) => {
  ...
  const mergedAt = sourceMetadata.merged_at
    ? new Date(sourceMetadata.merged_at as string).toLocaleDateString()
    : null;
          ...
          <div className="space-y-2 flex-1">
            ...
            <div className="text-sm text-amber-700">
              {isWebsiteToMarketplace && (
                <p>
                  This person first submitted via <strong>website</strong>, then later requested
                  access through the <strong>marketplace</strong>. The requests have been
                  automatically merged.
                </p>
              )}
              {isMarketplaceToWebsite && (
                <p>
                  This person first requested via <strong>marketplace</strong>, then a lead was
                  later imported from the <strong>website</strong>. The lead has been linked to the
                  existing request.
                </p>
              )}
              {mergedAt && (
