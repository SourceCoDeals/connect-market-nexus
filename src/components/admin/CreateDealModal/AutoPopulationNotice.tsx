...
export function AutoPopulationNotice({
  autoPopulatedFrom,
  form,
  onDismiss,
}: AutoPopulationNoticeProps) {
  return (
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 space-y-1">
          <p className="text-sm font-medium text-blue-900 dark:text-blue-100">
            📋 Auto-populated from{' '}
            {autoPopulatedFrom.source === 'user' ? 'user profile' : 'company profile'}
          </p>
          <p className="text-xs text-blue-700 dark:text-blue-300">
            Contact details were automatically filled using data from{' '}
            <strong>{autoPopulatedFrom.name}</strong> ({autoPopulatedFrom.email})
          </p>
        </div>
        <button
          type="button"
