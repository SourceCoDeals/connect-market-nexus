  ...
  const renderPathSequence = (path: PathSequence, index: number) => {
    ...
    return (
      <div key={path.path} className="group">
        <div className="flex items-center gap-3 py-2.5 hover:bg-muted/30 rounded-lg px-2 -mx-2 transition-colors">
          {/* Rank */}
          <span className="text-xs font-medium text-muted-foreground w-5 shrink-0 tabular-nums">
            {index + 1}
          </span>
          ...
          <div className="flex items-center gap-1 flex-1 min-w-0 overflow-hidden">
            {steps.map((step, stepIndex) => (
              <div key={stepIndex} className="flex items-center">
                <div 
                  className="px-2 py-1 rounded text-xs font-medium bg-muted text-foreground whitespace-nowrap"
                  title={step}
                >
                  {getPageLabel(step)}
                </div>
                {stepIndex < steps.length - 1 && (
                  <ArrowRight className="h-3 w-3 mx-1 text-muted-foreground/50 shrink-0" />
                )}
              </div>
            ))}
          </div>
