...
export function PipelineDetailBuyer({ deal }: PipelineDetailBuyerProps) {
  ...
  const getBuyerTypeLabel = (buyerType?: string, contactRole?: string) => {
    ...
    if (normalized.includes('independentsponsor') || normalized.includes('sponsor'))
                ...
                <div className="space-y-4">
                  ...
                  {profile.ideal_target_description && (
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-foreground">
                        {String(profile.ideal_target_description)}
                      </p>
                    </div>
                  )}
                  ...
                  {profile.specific_business_search && (
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-foreground">
                        {String(profile.specific_business_search)}
                      </p>
                    </div>
                  )}
                  ...
                  {profile.deploying_capital_now && (
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-foreground">
                        {String(profile.deploying_capital_now)}
                      </p>
                    </div>
                  )}
                  ...
                  {profile.buyer_type &&
                    getRelevantFieldsForBuyerType(profile.buyer_type as BuyerType)
                      .filter(
                        (field) =>
                          ![
                            'email',
                ...
                <TabsTrigger value="direct" className="text-xs">
                  Direct (
                  {
                    connectionRequests.filter(
                      (r: { id: string }) =>
                        !associatedRequests.some((a: { id: string }) => a.id === r.id),
                    ).length
                  }
                  )
                </TabsTrigger>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <p className="text-sm text-foreground">
                  {getBuyerTypeLabel(
                    deal.buyer_type ?? profile?.buyer_type ?? undefined,
                    deal.contact_role ?? undefined,
                  )}
                </p>
              </div>
