  ...
  const activities: ActivityItem[] = [
    ...
    ...realActivities.map((activity) => ({
      ...
      user: activity.admin?.email
        ? `${activity.admin.first_name || ''} ${activity.admin.last_name || ''}`.trim() ||
              ...
              <div className="space-y-1">
                {sortedActivities.map((activity, index) => {
                  ...
                  return (
                    <div
                      key={activity.id}
                      className="py-4 border-b border-border/10 last:border-b-0"
                    >
                          ...
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${
                              activity.type === 'stage_change'
                                ? 'bg-primary'
                                : ['nda_status_changed', 'fee_agreement_status_changed'].includes(
                                      activity.type,
                                    )
                                  ? 'bg-emerald-500'
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 border border-border/40 rounded-lg">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Since{' '}
                {deal.deal_stage_entered_at
                  ? format(new Date(deal.deal_stage_entered_at), 'MMM d, yyyy')
                  : 'Unknown'}
              </p>
            </div>
          </div>
