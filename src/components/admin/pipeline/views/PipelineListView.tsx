  ...
  return (
    <div className="flex-1 overflow-auto p-4">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {pipeline.selectedDeals.length > 0 && (
          <div className="flex items-center gap-2">
            <BulkActionsDropdown pipeline={pipeline} />
          </div>
        )}
      </div>
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        ...
                        pipeline.handleMultiSelect(
                          pipeline.selectedDeals.filter((id) => id !== deal.deal_id),
                        );
                      }
                    }}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        {(deal.deal_priority === 'high' || deal.deal_priority === 'urgent') && (
                          <Badge
                            className={`${getPriorityColor(deal.deal_priority)} h-5 px-2 text-xs`}
                          >
                            {deal.deal_priority}
                          </Badge>
                        )}
                      ...
                      {deal.deal_expected_close_date && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-muted-foreground">
                            {formatDistanceToNow(new Date(deal.deal_expected_close_date), {
                              addSuffix: true,
                            })}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {deal.nda_status === 'signed' && (
                        <Badge
                          variant="outline"
                          className="h-4 px-1 text-xs bg-green-50 text-green-700 border-green-200"
                        >
                          NDA
                        </Badge>
                      )}
                      {deal.fee_agreement_status === 'signed' && (
                        <Badge
                          variant="outline"
                          className="h-4 px-1 text-xs bg-blue-50 text-blue-700 border-blue-200"
                        >
                          Fee
                        </Badge>
                      )}
                      {deal.under_loi && (
                        <Badge
                          variant="outline"
                          className="h-4 px-1 text-xs bg-purple-50 text-purple-700 border-purple-200 font-semibold"
                        >
                          Under LOI
                        </Badge>
                      )}
                      {deal.pending_tasks > 0 && (
                        <Badge
                          variant="outline"
                          className="h-4 px-1.5 text-xs bg-red-50 text-red-700 border-red-200 gap-0.5"
                        >
