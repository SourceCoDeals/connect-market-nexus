...
const BUYER_TYPE_CONFIG: Record<string, { label: string; color: string }> = {
  ...
  pe_firm: { label: 'Private Equity', color: 'bg-[#1B3A6B]/10 text-[#1B3A6B] border-[#1B3A6B]/20' },
  ...
  strategic: { label: 'Strategic', color: 'bg-[#1A6B3A]/10 text-[#1A6B3A] border-[#1A6B3A]/20' },
  ...
  individual: { label: 'Individual', color: 'bg-[#4A4A4A]/10 text-[#4A4A4A] border-[#4A4A4A]/20' },
};
...
export function BuyerTypeBadge({ buyerType, isPeBacked, className }: BuyerTypeBadgeProps) {
  ...
  const effectiveType =
    (buyerType === 'corporate' || buyerType === 'strategic') && isPeBacked
      ? 'platform_co'
      : buyerType;
