...
const BuyerCard = ({ request }: { request: AdminConnectionRequest }) => {
            ...
            <div className="flex items-center gap-2">
              <h3 className="font-semibold text-sm">
                {request.user?.first_name
                  ...
                  : request.lead_name || request.lead_email || 'Unknown'}
              </h3>
              <StatusBadge status={request.status} />
            </div>
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span>{format(new Date(request.created_at), 'MMM d, yyyy')}</span>
              {request.source && request.source !== 'marketplace' && (
      ...
      <CardContent className="pt-0 space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="truncate">{request.user?.email || request.lead_email || '—'}</span>
          </div>
          {(request.user?.company || request.lead_company) && (
            <div className="flex items-center gap-2 text-xs">
              <Building2 className="h-3 w-3 text-muted-foreground" />
              <span className="truncate">{request.user?.company || request.lead_company}</span>
            </div>
          )}
        </div>
          ...
          <div className="space-y-3 pt-2 border-t border-border/50">
            {request.user_message && (
              <div className="text-xs">
                ...
                <div className="bg-muted/50 rounded p-2 text-xs">{request.user_message}</div>
              </div>
            )}
            ...
            <div className="text-xs space-y-1 pt-2 border-t border-border/50">
              ...
              {request.status === 'rejected' && (
                <div className="text-destructive">
                  ...
                  {request.rejected_at &&
                    ` on ${format(new Date(request.rejected_at), 'MMM d, yyyy')}`}
                </div>
              )}
              {request.status === 'on_hold' && (
                <div className="text-warning">
                  ...
                  {request.on_hold_at &&
                    ` on ${format(new Date(request.on_hold_at), 'MMM d, yyyy')}`}
                </div>
              )}
            </div>
          )}
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-16">
          ...
          <h3 className="text-xl font-semibold text-muted-foreground mb-2">No buyers found</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {requests.map((request) => (
          <BuyerCard key={request.id} request={request} />
        ))}
      </div>
    </div>
  );
