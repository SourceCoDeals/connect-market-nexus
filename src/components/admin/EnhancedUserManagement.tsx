...
import { Search, Download, UserCheck, Calculator } from 'lucide-react';
...
import { useBatchCalculateBuyerFitScores } from '@/hooks/admin/use-buyer-fit-score';
import { BUYER_FIT_TIERS } from './BuyerFitScoreBadge';
...
export function EnhancedUserManagement({
  ...
  const [platformSignalOnly, setPlatformSignalOnly] = useState(false);
  const [sortBy, setSortBy] = useState<string>('newest');
  const [selectedUsers, setSelectedUsers] = useState<string[]>([]);
  const batchCalculate = useBatchCalculateBuyerFitScores();
  ...
  const filteredUsers = useMemo(() => {
    let result = users.filter((user) => {
      ...
      const matchesTier =
        buyerTierFilter === 'all' ||
        (buyerTierFilter === 'not_scored' && !user.buyer_fit_tier) ||
        (buyerTierFilter !== 'not_scored' && user.buyer_fit_tier === parseInt(buyerTierFilter));

      const matchesPlatform = !platformSignalOnly || user.platform_signal_detected === true;
    ...
    if (sortBy === 'score_desc') {
      result = [...result].sort((a, b) => (b.buyer_fit_score ?? -1) - (a.buyer_fit_score ?? -1));
    } else if (sortBy === 'score_asc') {
      result = [...result].sort((a, b) => (a.buyer_fit_score ?? 999) - (b.buyer_fit_score ?? 999));
  ...
  const exportData = () => {
    const csvData = filteredUsers.map((user) => ({
      ...
      // Buyer Fit Score
      'Buyer Fit Score': user.buyer_fit_score ?? '',
      'Buyer Fit Tier': user.buyer_fit_tier
        ? `Tier ${user.buyer_fit_tier} - ${BUYER_FIT_TIERS[user.buyer_fit_tier]?.label || ''}`
        : '',
      'Platform Signal': user.platform_signal_detected ? 'Yes' : 'No',
      'Score Last Calculated': user.buyer_fit_score_last_calculated || '',
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6 pb-6 border-b">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => batchCalculate.mutate('unscored')}
              disabled={batchCalculate.isPending}
              className="gap-2 h-9"
            >
              <Calculator className="h-4 w-4" />
              <span className="hidden sm:inline">
                {batchCalculate.isPending ? 'Scoring...' : 'Score All Buyers'}
              </span>
            </Button>
            <Button
              variant="outline"
        ...
        <div className="space-y-5">
          ...
          {/* Row 2: Buyer Fit Score filters */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            <div>
              <Label className="text-sm font-medium text-muted-foreground">Buyer Tier</Label>
              <Select value={buyerTierFilter} onValueChange={setBuyerTierFilter}>
                <SelectTrigger className="h-10 border-border/60 focus:border-primary/40 transition-colors">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex items-end pb-1">
              <div className="flex items-center gap-2">
                <Switch
                  id="platform-signal"
                  checked={platformSignalOnly}
                  onCheckedChange={setPlatformSignalOnly}
                />
                <Label
                  htmlFor="platform-signal"
                  className="text-sm font-medium text-muted-foreground cursor-pointer"
                >
                  Platform/Add-On only
                </Label>
              </div>
            </div>
          </div>
