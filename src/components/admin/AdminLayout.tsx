    ...
    >
      ...
      {!collapsed && (
        <>
          ...
          {badge && badge > 0 && (
            <Badge 
              className="h-5 min-w-[20px] px-1.5 flex items-center justify-center text-[10px] font-semibold bg-status-required text-status-required-foreground border-status-required"
            >
              {badge > 9 ? '9+' : badge}
            </Badge>
          )}
      ...
      {collapsed && badge && badge > 0 && (
        <div className="absolute top-1 right-1 h-2 w-2 bg-status-required rounded-full" />
      )}
