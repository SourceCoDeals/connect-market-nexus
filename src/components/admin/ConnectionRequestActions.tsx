 ...
 *   - pipeline/ConversationThread.tsx  (message thread + composer)
 *   - pipeline/SidebarCard.tsx         (sidebar card wrapper)
 *   - hooks/admin/use-connection-request-actions.ts (NDA/fee status + access logic)
 */
...
import { ConversationThread } from './pipeline/ConversationThread';
import { SidebarCard } from './pipeline/SidebarCard';
import { useConnectionRequestActionsState } from '@/hooks/admin/use-connection-request-actions';
...
export function ConnectionRequestActions({
  ...
  // Extracted hook: NDA/fee status, access record, access toggle logic
  const {
    firmInfo,
    accessRecord,
    hasFeeAgreement,
    ...
    requestAccessToggle,
    confirmAccessToggle,
  } = useConnectionRequestActionsState(user, listing?.id, requestId);
              ...
              <p className="text-sm font-bold text-orange-800">
                ...
                {flaggedAssignedToAdmin && (
                  <span className="font-normal text-orange-700">
                    {' '}
                    &mdash; assigned to {flaggedAssignedToAdmin.first_name}{' '}
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-1">
                          {req.listing?.revenue
                            ...
                            : 'N/A'}{' '}
                          &middot; {req.listing?.location || 'N/A'}
                        </p>
                      </div>
                      <span
