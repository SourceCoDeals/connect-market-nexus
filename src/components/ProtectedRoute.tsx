...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ 
  ...
  // Add a timeout to prevent infinite loading - simplified
  useEffect(() => {
  ...
  // Simplified loading state - max 3 seconds
  if ((isLoading || !authChecked) && waitTime < 3) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 min-h-screen">
        ...
        {waitTime > 1 && (
          <p className="text-sm text-muted-foreground">Please wait... ({waitTime}s)</p>
        )}
      </div>
    );
  }
  ...
  if (user) {
    ...
    // If email is not verified, redirect to pending approval
    if (!isEmailVerified) {
