...
const AcquisitionTypeBadge = ({ type, className = '' }: AcquisitionTypeBadgeProps) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-sourceco ${className}`}
    >
      <Icon className="w-3 h-3 text-sourceco-foreground" strokeWidth={2.5} />
      <span className="text-[11px] font-semibold text-sourceco-foreground uppercase tracking-[0.05em] leading-none">
        {label}
      </span>
    </div>
  );
