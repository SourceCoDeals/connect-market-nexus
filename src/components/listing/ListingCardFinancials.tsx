...
const ListingCardFinancials = ({ 
  ...
  return (
    <div className={
      viewType === "grid" 
        ? "grid grid-cols-2 sm:grid-cols-4 items-start gap-x-6 py-4 border-y border-slate-200/60"
        : "grid grid-cols-2 gap-x-6 gap-y-5 px-5 py-4 border-y border-slate-200/30"
    }>
      {/* Revenue */}
      <div className="flex flex-col justify-between">
        <p className={`text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-500 ${viewType === "grid" ? "mb-1" : "mb-2"}`}>
          ANNUAL REVENUE
        </p>
        <p className={`${viewType === "grid" ? "text-[20px]" : "text-[20px]"} font-normal text-slate-900 tracking-[-0.025em]`}>
