...
interface PassConfirmDialogProps {
  ...
  isBulk?: boolean;
  bulkCount?: number;
  onBulkConfirm?: (reason: string, category: string) => void;
  isLoading?: boolean;
}
...
export const PassConfirmDialog = ({
  ...
  const handleConfirm = () => {
    ...
    const reasonLabel = notes && selectedReason === "other" ? notes : reason?.label || "Other";

    if (isBulk && onBulkConfirm) {
      onBulkConfirm(reasonLabel, selectedReason);
    } else {
      onConfirm(reasonLabel, selectedReason);
    }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {isBulk
              ? `This will mark ${bulkCount} selected buyers as passed. Help improve future matching by selecting a reason.`
        ...
        <div className="py-4 space-y-4">
          <RadioGroup value={selectedReason} onValueChange={setSelectedReason}>
            {PASS_REASONS.map((reason) => (
              ...
              >
                <RadioGroupItem value={reason.value} id={`pass-${reason.value}`} className="mt-0.5" />
                <Label htmlFor={`pass-${reason.value}`} className="cursor-pointer flex-1">
          ...
          <div>
            <Label htmlFor="pass-notes" className="text-sm font-medium">
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          >
            {isLoading ? "Saving..." : isBulk ? `Pass on ${bulkCount} Buyers` : "Confirm Pass"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
