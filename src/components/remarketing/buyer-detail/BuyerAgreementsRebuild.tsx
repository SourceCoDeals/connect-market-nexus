...
import { Check, X, FileSignature, Shield, Users, Download, Clock, Send, RefreshCw, Eye, AlertTriangle } from "lucide-react";
...
function getDocuSealStatusBadge(status: string | null) {
  ...
  const map: Record<string, { label: string; className: string; icon?: React.ReactNode }> = {
    ...
    sent: { label: "Sent", className: "bg-blue-100 text-blue-800", icon: <Send className="h-3 w-3 mr-0.5" /> },
    viewed: { label: "Viewed", className: "bg-purple-100 text-purple-800", icon: <Eye className="h-3 w-3 mr-0.5" /> },
    started: { label: "Started", className: "bg-purple-100 text-purple-800" },
    completed: { label: "Signed", className: "bg-green-100 text-green-800", icon: <Check className="h-3 w-3 mr-0.5" /> },
    signed: { label: "Signed", className: "bg-green-100 text-green-800", icon: <Check className="h-3 w-3 mr-0.5" /> },
    declined: { label: "Declined", className: "bg-red-100 text-red-800", icon: <X className="h-3 w-3 mr-0.5" /> },
    expired: { label: "Expired", className: "bg-orange-100 text-orange-800", icon: <AlertTriangle className="h-3 w-3 mr-0.5" /> },
  };
...
export function BuyerAgreementsRebuild({
  ...
  const ndaCanResend = firmAgreement && !firmAgreement.nda_signed && 
    (firmAgreement.nda_docuseal_status === 'pending' || firmAgreement.nda_docuseal_status === 'viewed' || 
     firmAgreement.nda_docuseal_status === 'declined' || firmAgreement.nda_docuseal_status === 'expired' ||
     firmAgreement.nda_docuseal_status === 'started');
  
  const feeCanResend = firmAgreement && !hasFeeAgreement && 
    (firmAgreement.fee_docuseal_status === 'pending' || firmAgreement.fee_docuseal_status === 'viewed' || 
     firmAgreement.fee_docuseal_status === 'declined' || firmAgreement.fee_docuseal_status === 'expired' ||
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              <p className="text-xs text-muted-foreground mb-1">DocuSeal Status</p>
              {getDocuSealStatusBadge(firmAgreement?.fee_docuseal_status ?? null) || <span className="text-sm text-muted-foreground">—</span>}
            </div>
            <div>
              <p className="text-xs text-muted-foreground mb-1">Signed Date</p>
              <p className="text-sm">
            ...
            <div>
              ...
              <p className="text-sm capitalize">
                ...
                {firmAgreement?.fee_agreement_scope ? ` · ${firmAgreement.fee_agreement_scope}` : ''}
              </p>
            </div>
          </div>
              ...
              {timelineEntries.map((entry) => (
                <div key={entry.id} className="flex items-start gap-3 py-2 border-b last:border-0 text-sm">
                  ...
                  <Badge variant="outline" className={`text-[10px] shrink-0 ${
                    entry.type === 'webhook' ? 'bg-blue-50 text-blue-700' : 'bg-muted text-muted-foreground'
                  }`}>
                    {entry.type === 'webhook' ? 'DocuSeal' : 'Status'}
                  </Badge>
                  <div className="flex-1 min-w-0">
                    <span className="capitalize">{entry.label}</span>
                    {entry.notes && <p className="text-muted-foreground text-xs mt-0.5">{entry.notes}</p>}
                  </div>
                </div>
              ))}
            </div>
