 ...
 interface DealEnrichmentSummaryDialogProps {
   open: boolean;
   onOpenChange: (open: boolean) => void;
   summary: DealEnrichmentSummary | null;
   onRetryFailed?: () => void;
 }
 
 export const DealEnrichmentSummaryDialog = ({
   open,
   onOpenChange,
   summary,
   onRetryFailed
 }: DealEnrichmentSummaryDialogProps) => {
                   ...
                   {summary.errors.map((err, idx) => (
                     ...
                     >
                       <div className="font-medium text-red-700 dark:text-red-400 truncate">
                         {err.dealName || `Deal ${err.listingId.slice(0, 8)}...`}
                       </div>
           ...
           {summary.errors.length === 0 && summary.successful > 0 && (
             <div className="flex items-center gap-3 p-4 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-700 dark:text-emerald-400">
               ...
               <span className="text-sm font-medium">
                 All {summary.successful} deals were enriched successfully!
               </span>
             </div>
           )}
         </div>
 ...
 export default DealEnrichmentSummaryDialog;