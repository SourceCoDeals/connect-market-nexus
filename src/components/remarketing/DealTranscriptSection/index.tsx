...
import { FileText, Plus, ChevronDown, ChevronUp, Sparkles, Loader2, RefreshCw, Trash2, AlertTriangle } from "lucide-react";
...
export function DealTranscriptSection({ dealId, transcripts, isLoading, dealInfo, contactEmail, contactEmails, contactName, companyName, onSyncComplete, onTranscriptLinked }: DealTranscriptSectionProps) {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button size="sm" variant="outline" className="gap-2" disabled={actions.isEnriching}>
          {actions.isEnriching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
          Enrich
  ...
  const addTranscriptDialog = (
    <Dialog open={actions.isAddDialogOpen} onOpenChange={(open) => { actions.setIsAddDialogOpen(open); if (!open) actions.resetForm(); }}>
      <DialogTrigger asChild>
        <Button size="sm" className="gap-2"><Plus className="h-4 w-4" /> Add Transcript</Button>
      </DialogTrigger>
      <TranscriptAddDialog
        transcriptTitle={actions.transcriptTitle} onTranscriptTitleChange={actions.setTranscriptTitle}
        transcriptUrl={actions.transcriptUrl} onTranscriptUrlChange={actions.setTranscriptUrl}
        callDate={actions.callDate} onCallDateChange={actions.setCallDate}
        newTranscript={actions.newTranscript} onNewTranscriptChange={actions.setNewTranscript}
        fileInputRef={actions.fileInputRef}
        selectedFiles={actions.selectedFiles} onSelectedFilesChange={actions.setSelectedFiles}
        isMultiFileMode={actions.isMultiFileMode} onFileUpload={actions.handleFileUpload}
        addMutationPending={actions.addMutation.isPending} onAddMutate={() => actions.addMutation.mutate()}
        processingProgress={actions.processingProgress}
        addMode={actions.addMode} onAddModeChange={actions.setAddMode}
        firefliesEmail={actions.firefliesEmail} onFirefliesEmailChange={actions.setFirefliesEmail}
        firefliesSearching={actions.firefliesSearching} onFirefliesSearch={actions.handleFirefliesSearch}
        firefliesResults={actions.firefliesResults}
        selectedFirefliesIds={actions.selectedFirefliesIds}
        onToggleFirefliesId={(id) => {
          actions.setSelectedFirefliesIds(prev => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; });
        }}
        ...
        onClose={() => { actions.setIsAddDialogOpen(false); actions.resetForm(); }}
      />
    </Dialog>
  );

  const enrichmentResultDialog = (
    <SingleDealEnrichmentDialog open={actions.showEnrichmentDialog} onOpenChange={actions.setShowEnrichmentDialog} result={actions.enrichmentResult} onRetry={() => actions.handleEnrichDeal(false)} />
  );
  ...
  const linkPanel = (
    <FirefliesLinkPanel
      ...
      ffFileInputRef={ff.ffFileInputRef} ffUploading={ff.ffUploading} onFfFileUpload={ff.handleFfFileUpload}
      ffQuery={ff.ffQuery} onFfQueryChange={ff.setFfQuery}
      ffSearchLoading={ff.ffSearchLoading} onFfQuickSearch={ff.handleFfQuickSearch}
      ffResults={ff.ffResults} ffLinking={ff.ffLinking} onLinkSearchResult={ff.handleLinkSearchResult}
    />
  );

  if (isLoading) {
    return <Card><CardHeader className="py-3"><Skeleton className="h-5 w-32" /></CardHeader></Card>;
  }
      ...
      <Card>
        <CardHeader className="py-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg flex items-center gap-2"><FileText className="h-5 w-5" /> Call Transcripts</CardTitle>
            <div className="flex items-center gap-2">{enrichButton}{addTranscriptDialog}</div>
          </div>
        </CardHeader>
        {actions.isEnriching && (
          <CardContent className="py-3 pt-0">
            <EnrichmentProgressCard enrichmentPhase={actions.enrichmentPhase} enrichmentProgress={actions.enrichmentProgress} primaryCounter />
          </CardContent>
        )}
  ...
  return (
    <Card>
      <Collapsible open={actions.isListExpanded} onOpenChange={actions.setIsListExpanded}>
          ...
          <div className="flex items-center justify-between">
            <CollapsibleTrigger asChild>
              <div className="flex items-center gap-2 cursor-pointer">
                ...
                {actions.isListExpanded ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}
              </div>
            </CollapsibleTrigger>
            <div className="flex items-center gap-2">{enrichButton}{addTranscriptDialog}</div>
          </div>
        ...
        {actions.isEnriching && (
          <CardContent className="py-3 pt-0">
            <EnrichmentProgressCard enrichmentPhase={actions.enrichmentPhase} enrichmentProgress={actions.enrichmentProgress} />
          </CardContent>
        )}
        <CollapsibleContent>
                  ...
                  <div className="flex items-center justify-between bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">
                    ...
                    <Button size="sm" variant="outline" className="h-7 text-xs"
                      onClick={async () => {
                        ...
                        const { error } = await supabase.from('deal_transcripts').delete().in('id', allIds);
                        if (error) toast.error('Failed to delete');
                        else { toast.success(`Cleaned up ${allIds.length} transcript${allIds.length > 1 ? 's' : ''}`); queryClient.invalidateQueries({ queryKey: ['remarketing', 'deal-transcripts', dealId] }); }
                      }}
                    >
            ...
            {transcripts.length > 1 && (
              <div className="flex items-center justify-between px-3 py-2 border rounded-lg bg-muted/30">
                <div className="flex items-center gap-2">
                  <Checkbox checked={actions.selectedTranscriptIds.size === transcripts.length} onCheckedChange={() => actions.toggleAllTranscripts(transcripts)} />
                  <span className="text-xs text-muted-foreground">{actions.selectedTranscriptIds.size > 0 ? `${actions.selectedTranscriptIds.size} selected` : 'Select all'}</span>
                </div>
                {actions.selectedTranscriptIds.size > 0 && (
                  <Button size="sm" variant="destructive" className="h-7 text-xs gap-1"
                    onClick={() => { if (confirm(`Delete ${actions.selectedTranscriptIds.size} transcript${actions.selectedTranscriptIds.size > 1 ? 's' : ''}?`)) actions.bulkDeleteMutation.mutate(Array.from(actions.selectedTranscriptIds)); }}
