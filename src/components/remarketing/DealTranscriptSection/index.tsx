 ...
 *
 * Renders and manages all call transcripts linked to a specific deal
 * (listing). Supports Fireflies sync, manual upload, AI enrichment,
 * duplicate detection, and bulk operations.
 *
 * ## deal_transcripts join pattern
 *
 * This component queries the `deal_transcripts` table filtered by
 * `listing_id` (the deal ID) only -- it does **not** filter by
 * `buyer_id`. This is intentional: the component displays *every*
 * transcript associated with a deal, regardless of which buyer or
 * contact participated, so admins have a complete view of all calls
 * for a given deal.
 *
 * This contrasts with `useRecommendedBuyers`, which queries the
 * separate `call_transcripts` table using a dual-key join
 * (`listing_id` + `buyer_id`) to build per-buyer scorecards. That
 * hook needs both keys because it is ranking buyers *for* a specific
 * listing and must not mix in calls from unrelated deals.
 *
 * The two tables serve different purposes:
 * - `deal_transcripts`: admin-facing, deal-scoped transcript storage
 *   (uploaded files, Fireflies links, extracted intelligence).
 * - `call_transcripts`: analytics-oriented, buyer+listing-scoped
 *   records used for scoring and recommendation ranking.
 */
        ...
        onToggleAllFireflies={() => {
          if (actions.selectedFirefliesIds.size === actions.firefliesResults.length)
            ...
            actions.setSelectedFirefliesIds(
              new Set(actions.firefliesResults.map((r: any) => r.id)),
            );
        }}
