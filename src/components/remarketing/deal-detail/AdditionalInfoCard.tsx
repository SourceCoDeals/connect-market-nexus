...
export const AdditionalInfoCard = ({
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        keyRisks: editedKeyRisks,
        technologySystems: editedTechnologySystems,
        realEstateInfo: editedRealEstateInfo,
        growthTrajectory: editedGrowthTrajectory,
      });
      setIsEditOpen(false);
  ...
  const openEdit = () => {
    ...
    setEditedKeyRisks(keyRisks || '');
    setEditedTechnologySystems(technologySystems || '');
    setEditedRealEstateInfo(realEstateInfo || '');
    setEditedGrowthTrajectory(growthTrajectory || '');
    setIsEditOpen(true);
  };
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <Textarea
                  ...
                  value={editedKeyRisks}
                  onChange={(e) => setEditedKeyRisks(e.target.value)}
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Uses ServiceTitan..."
                  value={editedTechnologySystems}
                  onChange={(e) => setEditedTechnologySystems(e.target.value)}
                  className="mt-1.5 min-h-[80px]"
                />
              </div>
              <div>
                <Label htmlFor="realEstateInfo">Real Estate</Label>
                <Textarea
                  id="realEstateInfo"
                  placeholder="Owned 10,000 sq ft..."
                  value={editedRealEstateInfo}
                  onChange={(e) => setEditedRealEstateInfo(e.target.value)}
                  className="mt-1.5 min-h-[80px]"
                />
              </div>
            </div>
            <div>
              <Label htmlFor="growthTrajectory">Growth Trajectory</Label>
              <Textarea
                id="growthTrajectory"
                placeholder="20% YoY growth..."
                value={editedGrowthTrajectory}
                onChange={(e) => setEditedGrowthTrajectory(e.target.value)}
                className="mt-1.5 min-h-[80px]"
              />
            </div>
            <div>
