/**
 * ValuationLeadOutreachDialog.tsx
 *
 * Owner-facing email outreach modal for valuation leads.
 * Mirrors LeadAgreementEmailDialog UX but with no attachments,
 * a template selector (intro / follow-up / custom), and editable subject.
 */
...
import {
  buildValuationOutreachEmail,
  type ValuationOutreachTemplateKind,
} from '@/lib/valuation-lead-outreach-template';
import { supabase } from '@/integrations/supabase/client';
import { useToast } from '@/hooks/use-toast';
import { useQueryClient } from '@tanstack/react-query';
import { isValidEmail } from '@/lib/email-validation';

interface ValuationLeadOutreachDialogProps {
  open: boolean;
...
export function ValuationLeadOutreachDialog({
  open,
  onOpenChange,
  valuationLeadId,
  leadEmail,
  ...
  hasSent,
}: ValuationLeadOutreachDialogProps) {
  const [isSending, setIsSending] = useState(false);
  const [selectedSender, setSelectedSender] = useState(
    DEAL_OWNER_SENDERS[1]?.email || DEAL_OWNER_SENDERS[0]?.email || '',
  );
  const [templateKind, setTemplateKind] = useState<ValuationOutreachTemplateKind>('intro');
  const [isEditing, setIsEditing] = useState(false);
  const [editedSubject, setEditedSubject] = useState('');
  const [editedBody, setEditedBody] = useState('');
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const { data: tracking } = useValuationLeadOutreachTracking(valuationLeadId);
  ...
  const emailPreview = useMemo(() => {
    return buildValuationOutreachEmail({
  ...
  // Reset edits whenever the template/sender/lead changes
  useEffect(() => {
    if (open) {
      setIsEditing(false);
  ...
  // When user picks "custom", auto-enter edit mode with the intro as a starting point
  useEffect(() => {
...
${editedBody
  ...
  .join('\n')}
      ...
      const { error } = await supabase.functions.invoke('send-valuation-lead-outreach', {
        body,
      });
      ...
      console.error('[ValuationLeadOutreachDialog] Send failed:', err);
      toast({
  ...
  // Prefer real name; otherwise the email's local part; otherwise "Lead".
  const displayName =
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col p-0 gap-0 overflow-hidden rounded-xl shadow-2xl border-border/50">
        ...
        <div className="flex-1 overflow-auto px-8 py-6 space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={templateKind}
              onValueChange={(v) => setTemplateKind(v as ValuationOutreachTemplateKind)}
            >
              <SelectTrigger className="h-9 text-sm border-border/60 bg-transparent hover:bg-muted/40 transition-colors duration-200">
                <SelectValue />
              </SelectTrigger>
                ...
                <SelectItem value="intro">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-muted-foreground">
                      First touch — references their valuation
                    </span>
                  </div>
                </SelectItem>
