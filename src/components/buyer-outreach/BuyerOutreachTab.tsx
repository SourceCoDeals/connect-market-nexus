...
export function BuyerOutreachTab({ dealId, dealName }: BuyerOutreachTabProps) {
  ...
  const handleResizeStart = useCallback(
    (e: React.MouseEvent, key: string) => {
      ...
      const onMove = (ev: MouseEvent) => {
        ...
        setColWidths((prev) => ({
          ...prev,
          [key]: Math.max(minW, resizingRef.current!.startW + diff),
        }));
      };
      const onUp = () => {
  ...
  const { data: buyers, isLoading: buyersLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: introEntries } = await supabase
        ...
        .select(
          'id, remarketing_buyer_id, contact_id, buyer_name, buyer_email, buyer_phone, buyer_linkedin_url, buyer_firm_name',
        )
        .eq('listing_id', dealId)
        .is('archived_at', null);
      ...
      if (!buyerIds.length && unresolvedIntroEntries.length === 0 && introContactIds.length === 0)
        return [];
      ...
      if (buyerIds.length > 0) {
        const { data } = await supabase
          .from('contacts')
          .select(
            'id, first_name, last_name, email, phone, linkedin_url, company_name, title, remarketing_buyer_id',
          )
      ...
      if (introsNeedingContacts.length > 0) {
        ...
        if (unresolvedIntroEntries.length > 0) {
          const companyNames = [
            ...new Set(unresolvedIntroEntries.map((e) => e.buyer_firm_name).filter(Boolean)),
          ];
          if (companyNames.length > 0) {
  ...
  const selectedContactIds = useMemo(() => selectedBuyers.map((b) => b.id), [selectedBuyers]);
  ...
  const handleAddToList = () => setAddToListOpen(true);
  const dealsForList: DealForList[] = selectedBuyers.map((b) => ({
    dealId: b.id,
    dealName: b.buyer_company_name || b.company_name || `${b.first_name} ${b.last_name}`,
    contactName: `${b.first_name} ${b.last_name}`.trim() || null,
        ...
        <CardHeader className="py-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              Buyer Contacts
              {buyers && (
                <span className="text-sm font-normal text-muted-foreground">({buyers.length})</span>
              )}
            </CardTitle>
              ...
              {sortedBuyers.map((buyer) => {
                ...
                return (
                  <div key={buyer.id}>
                    ...
                    >
                      <div
                        style={{ width: colWidths.checkbox, minWidth: 36 }}
                      ...
                      <div
                        style={{ width: colWidths.name, minWidth: 80 }}
                      ...
                      <div
                        style={{ width: colWidths.company, minWidth: 80 }}
                        className="min-w-0 shrink-0"
                      >
                      ...
                      <div
                        style={{ width: colWidths.title, minWidth: 60 }}
                        className="min-w-0 shrink-0"
                      >
                        <span className="text-xs text-muted-foreground truncate block">
                          {buyer.title || '—'}
                      ...
                      <div
                        style={{ width: colWidths.type, minWidth: 60 }}
                        className="min-w-0 shrink-0"
                      >
                        {buyer.buyer_type ? (
                          <BuyerTypeBadge
                      ...
                      <div
                        style={{ width: colWidths.email, minWidth: 80 }}
                        className="min-w-0 shrink-0"
                      >
                        {buyer.email ? (
                          <a
                      ...
                      <div
                        style={{ width: colWidths.phone, minWidth: 80 }}
                        className="min-w-0 shrink-0"
                      >
                        {buyer.phone ? (
                          <ClickToDialPhone
                      ...
                      <div
                        style={{ width: colWidths.channels, minWidth: 60 }}
                      ...
                      <div
                        style={{ width: colWidths.lastContact, minWidth: 60 }}
                    ...
                    {isExpanded && summary && (
                      <OutreachInlineDetail dealId={dealId} buyerId={buyer.id} summary={summary} />
                    )}
                  </div>
                );
              })}
