    ...
    async () => {
      try {
        ...
        if (profilesRes.error)
          logger.error('Error fetching profiles batch', 'useConnectionRequestsQuery', {
            error: String(profilesRes.error),
          });
        if (listingsRes.error)
          logger.error('Error fetching listings batch', 'useConnectionRequestsQuery', {
            error: String(listingsRes.error),
          });
