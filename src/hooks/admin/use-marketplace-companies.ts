  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select(
          'id, email, company, buyer_type, phone_number, website, linkedin_profile, business_categories, target_locations, updated_at',
        )
        ...
        .map((company) => {
          ...
          const primaryUser = company.users.reduce((best, current) => {
            ...
            if (
              ...
              new Date(current.updated_at) > new Date(best.updated_at)
            ) {
              return current;
            }
            return best;
          ...
          return {
            ...
            profileTemplate: {
              ...
              business_categories: primaryUser.business_categories as string[] | null,
              target_locations: primaryUser.target_locations as string[] | null,
