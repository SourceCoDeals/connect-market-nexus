...
export function useAdminEmail() {
  ...
  const sendConnectionApprovalEmail = async (request: AdminConnectionRequest) => {
    if (!request.user || !request.listing) {
      logger.error(
        'Cannot send connection approval email: missing user or listing data',
        'useAdminEmail',
      );
      return false;
    }
    ...
    try {
      ...
      if (error) {
        logger.error('Error sending connection approval notification', 'useAdminEmail', {
          error: String(error),
        });
        return false;
      }
      ...
      if (data && !data.success) {
        logger.error('Failed to send connection approval email', 'useAdminEmail', {
  ...
  const sendConnectionRejectionEmail = async (request: AdminConnectionRequest) => {
    if (!request.user || !request.listing) {
      logger.error(
        'Cannot send connection rejection email: missing user or listing data',
        'useAdminEmail',
      );
      return false;
    }
    ...
    try {
      ...
      if (error) {
        logger.error('Error sending connection rejection notification', 'useAdminEmail', {
          error: String(error),
        });
        return false;
      }
      ...
      if (data && !data.success) {
        logger.error('Failed to send connection rejection email', 'useAdminEmail', {
          message: String(data.message),
        });
        return false;
      }
  ...
  const sendAdminConnectionRequestEmail = async (request: AdminConnectionRequest) => {
    if (!request.user || !request.listing) {
      logger.error('Cannot send admin notification: missing user or listing data', 'useAdminEmail');
      return false;
    }
    ...
    try {
      ...
      if (error) {
        logger.error('Error sending admin notification', 'useAdminEmail', { error: String(error) });
        return false;
      }

      if (data && !data.success) {
        logger.error('Failed to send admin notification', 'useAdminEmail', {
          message: String(data.message),
        });
        return false;
      }
      ...
      logger.error('Failed to send admin notification', 'useAdminEmail', { error: String(error) });
      return false;
    }
  };
