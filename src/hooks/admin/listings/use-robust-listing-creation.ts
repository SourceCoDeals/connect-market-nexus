    ...
    mutationFn: async ({
      ...
      try {
        ...
        const databaseInsert: DatabaseListingInsert = {
          ...
          description_json: (listing.description_json as Json) || null,
          ...
          custom_sections: (listing.custom_sections as Json) || null,
          services: listing.services || null,
          growth_drivers: (listing.growth_drivers as Json) || null,
        ...
        const { data: insertedListing, error: insertError } = await supabase
          .from('listings')
          .insert(databaseInsert as TablesInsert<'listings'>)
          .select()
          .single();
 ...
 */
async function triggerDealAlertsForListing(listing: Tables<'listings'>): Promise<void> {
  try {
