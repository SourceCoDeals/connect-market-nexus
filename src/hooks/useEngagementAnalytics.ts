...
export function useEngagementAnalytics(timeRangeDays: number = 30) {
  return useQuery({
    ...
    queryFn: async (): Promise<EngagementAnalyticsData> => {
      ...
      const [analyticsResult, listingsResult, requestsResult, savedResult, referrerResult] =
        await Promise.all([
          supabase
            .from('listing_analytics')
            .select('listing_id, action_type, time_spent, scroll_depth')
            .gte('created_at', startDate.toISOString()),
          supabase.from('listings').select('id, title, category').eq('status', 'active'),
          supabase
      ...
      const scrollDepthCounts: Record<string, number> = {
        '0-25': 0,
        '26-50': 0,
        '51-75': 0,
        '76-100': 0,
