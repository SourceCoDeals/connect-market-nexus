  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const revenueOptimizations: RevenueOptimization[] = Array.from(categoryMap.entries())
        .map(([category, data]) => {
          const avgRevenue = data.listings.reduce((sum: number, l: { revenue: number }) => sum + l.revenue, 0) / data.listings.length;
      ...
      const competitiveIntelligence: CompetitiveIntelligence[] = Array.from(categoryMap.entries())
        .map(([category, data]) => ({
          ...
          competitor_pricing: data.listings.reduce((sum: number, l: { revenue: number }) => sum + l.revenue, 0) / data.listings.length * 1.1,
