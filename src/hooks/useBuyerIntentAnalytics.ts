  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BuyerIntentData> => {
      ...
      const capitalReadiness = {
        deployingNow: buyers.filter(
          (b) => b.deploying_capital_now === 'yes' || b.deploying_capital_now === 'Yes',
        ).length,
        raisingCapital: buyers.filter(
          (b) =>
            b.deploying_capital_now === 'raising' || b.deploying_capital_now === 'Raising capital',
        ).length,
        exploring: buyers.filter(
          (b) =>
            b.deploying_capital_now === 'exploring' || b.deploying_capital_now === 'Just exploring',
        ).length,
...
function formatBuyerType(type: string): string {
  const typeMap: Record<string, string> = {
    ...
    // Canonical snake_case values (from buyers.buyer_type)
