...
/**
 * Determine session status based on last activity timestamp.
 */
...
/**
 * Calculate session duration dynamically from session data.
 */
...
/**
 * Generate default coordinates for users without geo data.
...
/**
 * Normalize referrer to a readable source name.
 * Uses exact domain matching to prevent false positives (e.g., JWT tokens matching "t.co").
 */
...
/**
 * Resolve coordinates for a session, applying jitter for privacy.
 * Prefers stored lat/lon from IP-API, falls back to city lookup, then default.
 */
export function resolveSessionCoordinates(
  storedLat: number | null,
  storedLon: number | null,
  city: string | null,
  country: string | null,
  sessionId: string,
  ...
  if (typeof storedLat === 'number' && typeof storedLon === 'number') {
    ...
    return addJitter({ lat: storedLat, lng: storedLon }, sessionId);
  }

  // Fallback to city/country lookup
  const coordinates = getCoordinates(city, country);

  // Provide default coordinates for users without geo data
  // so they still appear on the globe (in Atlantic Ocean area)
  if (!coordinates) {
    return getDefaultCoordinates(sessionId);
  }

  return addJitter(coordinates, sessionId);
}
