    ...
    queryFn: async (): Promise<ExitAnalysisData> => {
      ...
      views.forEach((v) => {
        if (v.exit_page === true && v.page_path?.includes('/listing/')) {
          ...
          categoryExits[category].pages[v.page_path] =
            (categoryExits[category].pages[v.page_path] || 0) + 1;
        }
      });
      ...
      const avgPagesBeforeExit =
        sessionsWithExit.length > 0
          ? Math.round(
              (sessionsWithExit.reduce((sum, s) => sum + sessionPageCounts[s], 0) /
                sessionsWithExit.length) *
                10,
            ) / 10
          : 0;
