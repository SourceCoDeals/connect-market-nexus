...
export const useProductionErrorHandler = (componentName: string) => {
  ...
  const handleNetworkError = useCallback(
    (error: Error | string, endpoint?: string, metadata?: Record<string, unknown>) => {
      handleError(error, `API call to ${endpoint}`, 'medium', {
        ...metadata,
        type: 'network_error',
        endpoint,
      });
  ...
  const handleFormError = useCallback(
    (error: Error | string, formName?: string, fieldErrors?: Record<string, string>) => {
      handleError(error, `${formName || 'form'} submission`, 'low', {
        type: 'form_error',
        fieldErrors,
      });
    },
