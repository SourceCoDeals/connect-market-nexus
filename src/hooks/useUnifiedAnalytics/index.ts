    ...
    queryFn: async (): Promise<UnifiedAnalyticsData> => {
      ...
      const [
        ...
        supabase
          .from('user_sessions')
          .select(
            'id, session_id, user_id, visitor_id, referrer, original_external_referrer, blog_landing_page, utm_source, utm_medium, utm_campaign, utm_term, country, city, region, browser, os, device_type, session_duration_seconds, started_at, user_agent',
          )
          .eq('is_bot', false)
        ...
        supabase
          .from('user_sessions')
          .select(
            'id, session_id, user_id, visitor_id, referrer, session_duration_seconds, started_at, user_agent',
          )
          .eq('is_bot', false)
          .eq('is_production', true)
        ...
        supabase
          .from('profiles')
          .select(
            'id, first_name, last_name, company, buyer_type, referral_source, referral_source_detail, created_at',
          )
          .gte('created_at', startDateStr)
          .limit(200),
        supabase
      ...
      if (profileIds.length > 0) {
        const { data: firstSessionsData, error: firstSessionsError } = await supabase
          .from('user_sessions')
          .select(
            'id, session_id, user_id, visitor_id, referrer, original_external_referrer, blog_landing_page, utm_source, utm_medium, utm_campaign, utm_term, country, city, region, browser, os, device_type, started_at, user_agent',
          )
          .eq('is_bot', false)
          .eq('is_production', true)
      ...
      profileSessionsMap.forEach((sessions, userId) => {
        ...
        if (meaningfulSession)
          profileToFirstSession.set(userId, meaningfulSession as FirstSessionData);
      });

      // Build self-reported sources
      const selfReportedSourceMap: Record<
        string,
        { signups: number; connections: number; keywords: string[] }
      > = {};
      profiles.forEach((p) => {
      ...
      const { sessions: prevFilteredSessions } = filterAndDeduplicateSessions(
        rawPrevSessions as AnalyticsSession[],
      );
      ...
      const filteredConnections =
        filters.length > 0
          ...
          : connections;
      const filteredConnectionsWithMilestones =
        filters.length > 0
      ...
      const typedFilteredConnectionsWithMilestones =
        filteredConnectionsWithMilestones as unknown as AnalyticsConnection[];
      const typedUserToAttributionSession = userToAttributionSession as unknown as Map<
        string,
        AnalyticsSession
      >;
      ...
      const kpiData = computeKPIs(
        uniqueSessions,
        prevFilteredSessions,
        typedFilteredConnections,
        typedPrevConnections,
        filteredPageViews,
      ...
      const channels = computeChannels(
        uniqueSessions,
        typedFilteredConnections,
        typedFilteredProfiles,
        profileToFirstSession,
        typedUserToAttributionSession,
      );
      const referrers = computeReferrers(
        uniqueSessions,
        typedFilteredConnections,
        typedFilteredProfiles,
        profileToFirstSession,
      ...
      const { countries, regions, cities, geoCoverage } = computeGeography(
        uniqueSessions,
        typedFilteredConnections,
        typedFilteredProfiles,
        profileToFirstSession,
      ...
      const { browsers, operatingSystems, devices } = computeTechBreakdown(
        uniqueSessions,
        typedFilteredProfiles,
        profileToFirstSession,
        kpiData.currentVisitors,
      );
      const funnel = computeFunnel(
        ...
        filteredPageViews,
        typedFilteredConnections,
        typedFilteredConnectionsWithMilestones,
      ...
      const topUsers = computeTopUsers(
        uniqueSessions,
        typedFilteredConnectionsWithMilestones,
        filteredPageViews,
        currentSessionMap,
      ...
      const formattedDailyMetrics = formatDailyMetrics(
        filters,
        typedDailyMetrics,
        kpiData.dailyVisitorSets,
        kpiData.dailySessionCounts,
        kpiData.dailyConnectionCounts,
        startDate,
        endDate,
      );
      ...
      return {
        kpis: {
          visitors: {
            value: kpiData.currentVisitors,
            trend: kpiData.visitorsTrend,
            sparkline: kpiData.visitorSparkline,
          },
          sessions: {
            value: kpiData.currentSessionCount,
            trend: kpiData.sessionsTrend,
            sparkline: kpiData.sessionSparkline,
          },
          connections: {
            value: kpiData.currentConnections,
            trend: kpiData.connectionsTrend,
            sparkline: kpiData.connectionSparkline,
          },
