  ...
  return useQuery({
    ...
    queryFn: async (): Promise<UnifiedAnalyticsData> => {
      ...
      const [
        ...
        supabase
          .from('daily_metrics')
          .select('date, unique_visitors, total_sessions, connection_requests, bounce_rate')
      ...
      profileSessionsMap.forEach((sessions, userId) => {
        ...
        if (meaningfulSession) profileToFirstSession.set(userId, meaningfulSession);
      });
      ...
      const { sessions: prevFilteredSessions } = filterAndDeduplicateSessions(rawPrevSessions);
      ...
      const funnel = computeFunnel(
        ...
        filteredPageViews,
        filteredConnections,
        filteredConnectionsWithMilestones,
