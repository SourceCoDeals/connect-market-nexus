...
export function filterAndDeduplicateSessions(rawSessions: any[]) {
  const filtered = rawSessions.filter(
    (s) =>
...
export function applySessionFilters(
  ...
  if (filters.length > 0) {
    filters.forEach((filter) => {
      ...
      if (filter.type === 'campaign')
        result = result.filter((s) => s.utm_campaign === filter.value);
  ...
  if (pageFilter) {
    const matchingSessionIds = new Set(
      pageViews.filter((pv) => pv.page_path === pageFilter.value).map((pv) => pv.session_id),
    );
  ...
  return profiles.filter((p) => {
    ...
    for (const filter of filters) {
      if (filter.type === 'channel') {
        if (firstSession?.original_external_referrer) {
          const channel = categorizeChannel(
            firstSession.original_external_referrer,
        ...
        if (firstSession) {
          const sessionChannel = categorizeChannel(
      ...
      if (filter.type === 'referrer') {
        ...
        if (p.referral_source && p.referral_source.toLowerCase() + '.com' === filter.value)
          return true;
        return false;
      }
...
export function computeKPIs(
  ...
  // Trends
  const visitorsTrend =
    prevVisitors > 0 ? ((currentVisitors - prevVisitors) / prevVisitors) * 100 : 0;
  const sessionsTrend =
    prevSessionCount > 0 ? ((currentSessionCount - prevSessionCount) / prevSessionCount) * 100 : 0;
  ...
  const connectionsTrend =
    prevConnectionsCount > 0
      ? ((currentConnections - prevConnectionsCount) / prevConnectionsCount) * 100
      : 0;
  ...
  const bounceRate =
    sessionPageCounts.size > 0 ? (bouncedSessions / sessionPageCounts.size) * 100 : 0;
  ...
  const last7Days = Array.from({ length: 7 }, (_, i) =>
    format(subDays(endDate, 6 - i), 'yyyy-MM-dd'),
  );
