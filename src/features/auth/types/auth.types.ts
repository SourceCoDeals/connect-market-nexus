...
export const signupFormSchema = z
  .object({
    ...
    idealTargetDescription: z
      .string()
      .min(20, 'Please provide at least 20 characters describing your ideal targets'),
    businessCategories: z.array(z.string()).min(1, 'Please select at least one business category'),
    targetLocations: z.string().min(1, 'Please select at least one target location'),
    revenueRangeMin: z.string().optional(),
    revenueRangeMax: z.string().optional(),
    specificBusinessSearch: z.string().min(1, 'Specific business requirements are required'),
    // New common fields
    jobTitle: z.string().min(1, 'Job title is required'),

    // Corporate/PE fields
    estimatedRevenue: z.string().optional(), // required for corporate via refine
    fundSize: z.string().optional(), // required for PE/FO via refine
    investmentSize: z.string().optional(), // required for PE/FO via refine
    aum: z.string().optional(), // required for PE/FO via refine
    ...
    // New Step 4 fields
    dealIntent: z.string().min(1, 'Deal intent is required'),
  ...
  .refine(
    (data) => {
      ...
      if (data.buyerType === 'searchFund') {
        ...
        if (!data.anchorInvestorsSummary) return false;
        if (!data.searchStage) return false;
      }

      // Conditional validation for Private Equity
      if (data.buyerType === 'privateEquity') {
        if (!data.deployingCapitalNow) return false;
        if (!data.fundSize) return false;
        if (!data.investmentSize) return false;
        if (!data.aum) return false;
      }

      // Conditional validation for Corporate Development
      if (data.buyerType === 'corporate') {
        if (!data.dealSizeBand) return false;
        if (!data.estimatedRevenue) return false;
        if (!data.integrationPlan || data.integrationPlan.length === 0) return false;
        if (!data.corpdevIntent) return false;
      }
      ...
      if (data.buyerType === 'independentSponsor') {
        ...
        if (!data.backersSummary) return false;
        if (!data.deploymentTiming) return false;
      }
      ...
      if (data.buyerType === 'individual') {
        ...
        if (!data.maxEquityTodayBand) return false;
        if (!data.usesBankFinance) return false;
      }

      // Conditional validation for Advisor
      if (data.buyerType === 'advisor') {
        if (!data.onBehalfOfBuyer) return false;
        if (!data.mandateBlurb) return false;
        if (data.onBehalfOfBuyer === 'yes') {
          if (!data.buyerRole || !data.buyerOrgUrl) return false;
        }
      }
