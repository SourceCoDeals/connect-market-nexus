...
## 0. Core Design Philosophy: Speed-First, Depth-On-Demand

**The #1 design constraint for the AI Command Center is speed.** Every architectural decision flows from this principle: users get a fast, concise answer immediately, and can drill deeper on demand.

### 0.1 The Two-Speed Architecture

```
FAST PATH (< 2 seconds)                    DEEP PATH (< 10 seconds)
─────────────────────                       ──────────────────────
"How many active deals?"                    "Give me a full analysis of our
→ Haiku routes directly                      HVAC pipeline with buyer fit
→ Single tool call                           scores and transcript insights"
→ 1-2 sentence answer                      → Sonnet/Opus orchestrates
→ No streaming needed                       → 5-7 parallel tool calls
                                            → Structured multi-section response
                                            → Streamed progressively

"Who needs follow-up?"                      "Compare our Q1 vs Q2 pipeline
→ Haiku routes to Sonnet                     performance with close rates
→ 2-3 parallel tool calls                    and source attribution"
→ Prioritized bullet list                   → Opus deep analysis
→ First tokens in < 500ms                   → Cross-system aggregation
→ "Want me to go deeper on any?"            → Tables + trend analysis
```

### 0.2 Speed Design Principles

| Principle | Implementation |
|-----------|---------------|
| **Answer first, explain later** | Lead with the answer ("You have 12 active deals"), then offer detail ("Want the breakdown by stage?") |
| **Progressive disclosure** | Short answer → "Dive deeper?" prompt → expanded analysis on request |
| **Parallel everything** | Tool calls fire in parallel, not sequential, whenever possible |
| **Right-size the model** | 40% of queries can be answered by Haiku alone (< 1s). Don't use Sonnet when Haiku suffices. |
| **Prefetch user context** | Cache current user's deal ownership, tasks, and role on session start — don't fetch on every query |
| **Stream immediately** | First token appears in < 500ms. User sees progress, not a loading spinner. |
| **Truncate smartly** | Return top 5 results with "showing 5 of 47 — want more?" instead of dumping all 47 |
| **Skip the router when obvious** | If the user is on a deal page and asks "what's the score?", skip Haiku routing — it's obviously a deal query |
| **Cache hot queries** | "How many active deals?" doesn't change every 30 seconds. Cache for 60s. |
| **Fail fast, recover gracefully** | If a tool times out after 5s, return what you have: "Here's the deal info. I couldn't load transcripts — want me to try again?" |

### 0.3 Response Time Targets

| Query Complexity | Example | Target P50 | Target P95 | Model |
|-----------------|---------|-----------|-----------|-------|
| **Instant** | "How many active deals?" | < 1s | < 2s | Haiku direct |
| **Quick** | "Show me my pipeline" | < 1.5s | < 3s | Haiku + 1 tool |
| **Standard** | "Who needs follow-up?" | < 2.5s | < 5s | Sonnet + 2-3 tools |
| **Complex** | "Find HVAC buyers in FL across all sources" | < 4s | < 8s | Sonnet + 4-5 tools |
| **Deep** | "Morning briefing with full pipeline analysis" | < 8s | < 15s | Opus + 5-7 tools |

### 0.4 The "Dive Deeper" Pattern

Every response ends with contextual follow-up affordances. These are NOT generic suggestions — they are specific to the data just returned:

```
User: "What are my active deals?"

AI: You have 12 active deals. Top 3 by recent activity:
...
The follow-up suggestions are generated by the LLM based on the actual data returned, not hardcoded templates.

### 0.5 Speed Optimizations Stack

```
Layer 1: Client-side
  - Prefetch user context on login (deals, tasks, role)
  - Optimistic UI: show "thinking" state in < 100ms
  - Cache recent conversations for instant resume
  - Keyboard shortcut (Cmd+K) opens panel in < 50ms

Layer 2: Edge function
  - Skip router for contextual queries (deal page → deal query)
  - Parallel tool dispatch (fire all independent tools simultaneously)
  - Early streaming: start sending tokens before all tools complete
  - Response cache: 60s TTL for aggregate queries (counts, pipeline summary)

Layer 3: Database
  - Indexed queries: all tool queries hit indexed columns
  - Connection pooling: reuse Supabase client across tool calls
  - Query timeout: 5s per tool, partial results returned if exceeded
  - Materialized views: pipeline_summary, daily_metrics pre-computed

Layer 4: LLM
  - Haiku for 40% of queries (< 300ms LLM time)
  - Prompt caching: system prompt prefix cached across requests
  - Compact tool results: strip unnecessary fields before sending to LLM
  - Max 500 token responses for quick queries, 2000 for deep dives
```
...
| Tier | Model | Use Cases | % of Traffic | Latency Target |
|------|-------|-----------|-------------|---------------|
| **Tier 0: Cached** | None (cache hit) | Repeated aggregate queries within 60s TTL | ~10% | < 100ms |
| **Tier 1: Router** | claude-haiku-4-5 | Intent classification, tool selection, model routing. **Skipped when context is obvious** (e.g., deal page query). | ~60% (not all queries need routing) |< 300ms |
| **Tier 2: Quick** | claude-haiku-4-5 | Simple lookups ("How many active deals?"), single-tool queries, yes/no questions, status checks. **Haiku answers directly without handoff.** | ~40% | < 1s total |
| **Tier 3: Standard** | claude-sonnet-4 | Multi-tool queries, buyer search, deal analysis, follow-up identification. **Most queries land here.** | ~45% | < 3s total |
| **Tier 4: Deep** | claude-opus-4 | Complex synthesis (daily briefings, multi-source analysis, trend comparisons). **Only when user explicitly requests depth** or query requires 5+ data sources. | ~5% | < 10s total |

### 2.3 Router Implementation (Speed-Optimized)

The router has three paths, ordered by speed:

```typescript
// FAST PATH 1: Cache hit (< 100ms)
// Aggregate queries that don't change second-to-second
const CACHEABLE_PATTERNS = [
  /how many (active|total) deals/i,
  /pipeline (summary|overview|count)/i,
  /deal count/i,
];

// FAST PATH 2: Context bypass (skip Haiku routing entirely)
// When the user is on a specific page, the intent is obvious
function contextBypass(query: string, pageContext: PageContext): Route | null {
  if (pageContext?.type === 'deal') {
    // On a deal page — any query is about this deal
    if (/score|fit|buyer|match/i.test(query))
      return { category: 'BUYER_ANALYSIS', model_tier: 'STANDARD', tools_needed: ['get_top_buyers_for_deal', 'get_score_breakdown'] };
    if (/transcript|call|meeting|ceo|said/i.test(query))
      return { category: 'MEETING_INTEL', model_tier: 'STANDARD', tools_needed: ['search_transcripts'] };
    if (/activity|happened|update|status/i.test(query))
      return { category: 'DEAL_STATUS', model_tier: 'QUICK', tools_needed: ['get_deal_activities'] };
  }
  if (pageContext?.type === 'buyer') {
    return { category: 'BUYER_ANALYSIS', model_tier: 'STANDARD', tools_needed: ['get_buyer_profile'] };
  }
  return null; // Fall through to Haiku router
}

// STANDARD PATH: Haiku classification (< 300ms)
const ROUTER_PROMPT = `You are a fast query router for an M&A deal management system.
Your job is speed. Classify in the minimum tokens possible.
...
Model tier rules (ALWAYS pick the FASTEST tier that can handle it):
- QUICK (Haiku): Single-table lookups, yes/no, counts, simple status checks. DEFAULT CHOICE when unsure.
- STANDARD (Sonnet): Multi-table queries requiring 2-4 tool calls
- DEEP (Opus): ONLY for explicit requests for comprehensive analysis, briefings, or 5+ source synthesis

BIAS TOWARD QUICK. Most questions can be answered with 1-2 tool calls.

Respond with compact JSON only:
{"c":"CATEGORY","t":"QUICK|STANDARD|DEEP","tools":["tool1"],"ctx":true}`;

// c = category, t = tier, tools = tools needed, ctx = needs user context
// Compact format saves ~50 output tokens per classification = faster routing
```
...
1. **Speed above all:** Every tool has a 5-second hard timeout. If it can't answer in 5s, return partial results.
2. **Atomic operations:** Each tool does one thing well
3. **Structured output:** Tools return JSON, not free text
4. **Error handling:** Every tool returns `{ data, error, partial }` pattern — `partial: true` means timeout with partial results
5. **Smart defaults:** Default limit is 5 (not 50). Show top 5, mention total. User asks for more = second tool call.
6. **Parallel by default:** Tools declare their dependencies. Independent tools always fire in parallel.
7. **Two-depth mode:** Every tool supports `depth: "quick" | "full"`. Quick returns summary fields only. Full returns everything.

```typescript
// Example: query_deals in quick mode returns:
{ deals: [{ name, stage, last_activity, activity_count }], total: 47 }

// In full mode returns:
{ deals: [{ name, stage, last_activity, activity_count, owner, priority,
             tasks_overdue, revenue, ebitda, buyer_count, top_buyer }], total: 47 }
```
