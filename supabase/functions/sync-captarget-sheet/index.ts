...
async function rowToRecord(
  ...
  // Treat "-" and similar placeholder values as empty — they appear in sheets
  // where the company name cell was filled with a dash instead of left blank.
      ...
      for (let i = rowOffset; i < dataRows.length; i += BATCH_SIZE) {
        ...
        for (const record of batchRecords) {
          ...
          if (existingId) {
            ...
            const {
              status: _status,
              pushed_to_all_deals: _pushed,
              deal_source: _src,
              is_internal_deal: _internal,
              captarget_row_hash: _hash,
              ...updateFields
            } = record;
