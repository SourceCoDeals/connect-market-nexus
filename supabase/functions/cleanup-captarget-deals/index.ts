...
serve(async (req) => {
  ...
  try {
    ...
    if (!body.confirm) {
      return new Response(JSON.stringify({ error: 'Must pass { confirm: true } to run cleanup' }), {
        status: 400,
    ...
    while (hasMore) {
      const { data: deals, error: fetchErr } = await supabase
        .from('listings')
        .select(
          'id, internal_company_name, title, captarget_call_notes, main_contact_title, captarget_row_hash, industry, category',
        )
      ...
      const toExclude: Array<{
        deal: (typeof deals)[0];
        result: ReturnType<typeof checkCompanyExclusion>;
      }> = [];
      ...
      if (toExclude.length > 0 && !dryRun) {
        ...
        for (let i = 0; i < idsToDelete.length; i += DELETE_CHUNK) {
          ...
          const { error: delErr } = await supabase.from('listings').delete().in('id', chunk);
          if (delErr) {
