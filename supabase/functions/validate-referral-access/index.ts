  ...
  try {
    ...
    if (action === 'get-data') {
      ...
      const { data: listings } = await supabase
        .from('listings')
        .select(
          'id, title, internal_company_name, category, revenue, ebitda, full_time_employees, location, status, is_priority_target, website, deal_total_score, main_contact_name, main_contact_title, main_contact_email, main_contact_phone, linkedin_employee_count, linkedin_employee_range, needs_owner_contact, pushed_to_all_deals',
        )
      ...
      // Fetch pipeline + buyer engagement data per listing
      ...
      if (listingIds.length > 0) {
        ...
        for (const row of pipelineRows || []) {
          pipelineMap[row.listing_id] = (pipelineMap[row.listing_id] || 0) + 1;
        }
        ...
        for (const row of introRows || []) {
          introMap[row.listing_id] = (introMap[row.listing_id] || 0) + 1;
        }
      }

      // Annotate listings with pipeline and buyer intro counts
      const enrichedListings = (listings || []).map((l: Record<string, unknown>) => ({
        ...l,
        pipeline_deal_count: pipelineMap[l.id as string] || 0,
        buyer_intro_count: introMap[l.id as string] || 0,
      }));
