...
/**
 * send-valuation-lead-outreach
 *
 * Sends a 1:1 owner outreach email tied to a valuation_leads record.
 *
 * Email composition mirrors src/lib/valuation-lead-outreach-template.ts and
 * src/lib/valuation-lead-outreach-hooks.ts. Keep these three files in sync.
 *
 * Architecture: hook-routed micro-templates. The lead's signals (spread,
 * tenure, industry, margin, timing, tier) are scored, the highest-scoring
 * hook is picked deterministically, and that hook routes to its own 2 to 4
 * sentence note with its own cadence and signoff rhythm.
 *
...
type SpreadBand = 'tight' | 'typical' | 'wide' | null;
function spreadBand(low: number | null | undefined, high: number | null | undefined): SpreadBand {
...
function timingBand(exitTiming: string | null | undefined): TimingBand {
...
function pickTopHook(input: HookInputs): { kind: HookKind; score: number; reason: string } {
  ...
  if (sb === 'tight') candidates.push({ kind: 'spread_anomaly', score: 0.9, reason: 'tight' });
  else if (sb === 'wide') candidates.push({ kind: 'spread_anomaly', score: 0.85, reason: 'wide' });
  else if (sb === 'typical')
    candidates.push({ kind: 'spread_anomaly', score: 0.4, reason: 'typical' });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!valuationLeadId || !leadEmail) {
      return new Response(JSON.stringify({ error: 'Missing valuationLeadId or leadEmail' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    }
    ...
    if (!isValidEmail(leadEmail)) {
      console.log(
        `[send-valuation-lead-outreach] REJECT | invalid leadEmail="${leadEmail}" lead=${valuationLeadId}`,
      );
    ...
    console.log(
      `[send-valuation-lead-outreach] START | lead=${valuationLeadId} | to=${emailLc} | kind=${templateKind} | resend=${!!isResend}`,
    );
    ...
    try {
      const { data: suppressed } = await supabase
        ...
        .maybeSingle();
      if (suppressed) {
        console.log(`[send-valuation-lead-outreach] SKIP | suppressed=${emailLc}`);
        await supabase
    ...
    const resolvedSenderName = senderName || 'SourceCo Team';
    ...
    const emailResult = await sendEmail({
      templateName: 'valuation_owner_outreach',
      to: emailLc,
    ...
    console.log(
      `[send-valuation-lead-outreach] result | success=${emailResult.success} | hook=${hookKind || 'custom'} | emailId=${emailResult.emailId}`,
    );
    ...
    console.error('[send-valuation-lead-outreach] Error:', err);
