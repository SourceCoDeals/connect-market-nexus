  ...
  try {
    ...
    if (teaserError || !teaser) {
      return new Response(
        JSON.stringify({
          error:
            'No active Anonymous Teaser found for this deal. Upload a teaser before approving marketplace buyers.',
        }),
    ...
    if (!deal.project_name) {
      return new Response(
        JSON.stringify({
          error:
            'Deal does not have a project_name set. Set a project codename before distributing teasers.',
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    ...
    if (releaseError || !releaseLog) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to create release log' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
