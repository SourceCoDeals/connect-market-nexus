...
describe('scoreListingAgainstCriteria — keyword gate', () => {
  ...
  it('regression (Alpine 2026-04): summary-only "mechanical" match is blocked when primary industry contradicts', () => {
    // Auto body shops often say "mechanical repair" in their executive_summary.
    // Before this fix, an HVAC thesis with keyword "mechanical" scored them
    // as HVAC matches.
    const result = scoreListingAgainstCriteria(
      {
        ...
        categories: [],
        executive_summary:
          'Full-service auto body and mechanical repair shop specializing in collision work.',
        service_mix: 'collision repair, mechanical repairs, paint services',
      },
      makeCriteria({
        industry_label: 'HVAC',
        industry_keywords: ['hvac', 'heating', 'cooling', 'mechanical'],
      }),
    );
    expect(result.score).toBe(0);
  });

  it('summary-only match still scores (weak) when listing has no structured industry', () => {
    const result = scoreListingAgainstCriteria(
      {
        industry: null,
        categories: [],
        executive_summary: 'Commercial HVAC services and installation across the tri-state.',
      },
      makeCriteria({ industry_keywords: ['hvac'] }),
    );
    // 20 weak-industry + 25 national = 45 (no size signal)
    expect(result.score).toBeGreaterThan(0);
    expect(result.reasons.some((r) => /weak/.test(r))).toBe(true);
    expect(result.reasons.some((r) => /\[secondary\]/.test(r))).toBe(true);
  });

  it('primary-match reason is tagged [primary]', () => {
    const result = scoreListingAgainstCriteria(
      { industry: 'HVAC' },
      makeCriteria({ industry_keywords: ['hvac'] }),
    );
    expect(result.reasons.some((r) => /\[primary\]/.test(r))).toBe(true);
  });

  it('excluded keyword kills the match even when primary industry matches', () => {
    // Imagine a roll-up fund that likes HVAC but wants to skip anything
    // "franchise" — they add 'franchise' to excluded_keywords.
    const result = scoreListingAgainstCriteria(
      {
        industry: 'HVAC',
        executive_summary: 'A national HVAC franchise with 40 locations.',
      },
      makeCriteria({
        industry_keywords: ['hvac'],
        excluded_keywords: ['franchise'],
      }),
    );
    expect(result.score).toBe(0);
    expect(result.reasons.some((r) => /excluded/.test(r))).toBe(true);
  });

  it('excluded_keywords empty is a no-op', () => {
    const result = scoreListingAgainstCriteria(
      { industry: 'HVAC' },
      makeCriteria({ industry_keywords: ['hvac'], excluded_keywords: [] }),
    );
    expect(result.score).toBeGreaterThan(0);
  });
});
