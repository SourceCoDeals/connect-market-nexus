...
async function explainBuyerScore(
  ...
  const [scoreResult, buyerResult, dealResult, adjustmentsResult] = await Promise.all([
    ...
    supabase
      ...
      .eq('id', dealId)
      .single(),
    supabase.from('deal_scoring_adjustments').select('*').eq('listing_id', dealId).maybeSingle(),
  ]);
...
function buildExplanation(
  ...
  switch (field) {
    case 'geography_score': {
      ...
      const footprint =
        (buyer.geographic_footprint as string[] | undefined)?.join(', ') || 'none listed';
      return `${tier} match (${score}/100). Buyer HQ: ${buyerHQ}. Deal location: ${dealLoc}. Buyer footprint: ${footprint}.`;
    }
    case 'service_score': {
      const buyerSvcs =
        (buyer.target_services as string[] | undefined)?.slice(0, 5).join(', ') || 'none listed';
    ...
    case 'size_score': {
      const range =
        buyer.target_revenue_min || buyer.target_revenue_max
          ? `$${(Number(buyer.target_revenue_min) || 0) / 1e6}M–$${(Number(buyer.target_revenue_max) || 999) / 1e6}M`
          : 'no range set';
      const dealRev = deal?.revenue ? `$${(Number(deal.revenue) / 1e6).toFixed(1)}M` : 'unknown';
