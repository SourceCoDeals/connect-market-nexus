...
serve(async (req) => {
  ...
  try {
    ...
    if (!submissionId || !action) {
      return new Response(JSON.stringify({ error: 'submissionId and action required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const { data: submission, error: fetchError } = await supabase
      ...
      .select(
        'id, status, company_name, website, industry, revenue, ebitda, location, referral_partner_id, contact_name, contact_email, contact_phone',
      )
      .eq('id', submissionId)
      .single();
    ...
    if (submission.status !== 'pending') {
      return new Response(JSON.stringify({ error: 'Submission already reviewed' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ success: true, action: 'approved', listing }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (err) {
