...
const PLAIN_TEXT_TYPES = new Set(['text/csv']);
...
export async function extractTextFromDocument(
  ...
  try {
    ...
    if (arrayBuffer.byteLength > MAX_EXTRACTION_SIZE) {
      return {
        text: null,
        error: `File too large for AI extraction (${(arrayBuffer.byteLength / 1024 / 1024).toFixed(1)}MB, max ${MAX_EXTRACTION_SIZE / 1024 / 1024}MB)`,
      };
    }
