...
serve(async (req) => {
  ...
  try {
    ...
    const query = supabase
      ...
      .select(
        'buyer_id, composite_score, geography_score, size_score, service_score, status, pass_category, pass_reason, is_disqualified',
      )
    ...
    // Analyze patterns
    const approved = scores.filter((s: { status?: string }) => s.status === 'approved');
    const passed = scores.filter(
      (s: { status?: string }) => s.status === 'passed' || s.status === 'not_a_fit',
    );
    ...
    if (totalDecisions < 3) {
      return new Response(
        JSON.stringify({
          success: true,
          adjustments: null,
          message: 'Not enough decisions yet (need 3+)',
        }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    ...
    const servicePassRate =
      (passCategories['service'] || passCategories['services'] || 0) / totalPasses;
    ...
    const avgApprovedScore =
      approved.length > 0
        ? Math.round(
            approved.reduce(
              (s: number, r: { composite_score?: number }) => s + (r.composite_score || 0),
              0,
            ) / approved.length,
          )
        : 0;
    ...
    const { error: upsertError } = await supabase.from('deal_scoring_adjustments').upsert(
      {
        deal_id: listingId,
        ...adjustments,
      },
    ...
    return new Response(JSON.stringify({ success: true, adjustments, stats }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
