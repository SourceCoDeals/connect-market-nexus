-- 1. Add raw_body column to incoming_leads
ALTER TABLE public.incoming_leads ADD COLUMN IF NOT EXISTS raw_body jsonb;

-- 2. Replace merge_valuation_lead with version that includes readiness_score
CREATE OR REPLACE FUNCTION public.merge_valuation_lead(
  ...
  p_locations_count integer DEFAULT NULL,
...
BEGIN
  ...
  DO UPDATE SET
    ...
    readiness_score = COALESCE(EXCLUDED.readiness_score, valuation_leads.readiness_score),
    updated_at = now()
...
-- Drop the old 44-param version (same name, fewer params) if it exists as a separate overload
-- The CREATE OR REPLACE above handles the case where the signature matches exactly.
-- Since we added p_readiness_score, this is technically a NEW overload. Drop the old one.
