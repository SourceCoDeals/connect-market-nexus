-- Update handle_new_user trigger with proper type checks to fix array length error
...
BEGIN
  -- Safe array handling with type checking
  v_business_categories := CASE 
    WHEN raw->'business_categories' IS NOT NULL AND jsonb_typeof(raw->'business_categories') = 'array' THEN raw->'business_categories'
    WHEN raw->'businessCategories' IS NOT NULL AND jsonb_typeof(raw->'businessCategories') = 'array' THEN raw->'businessCategories'
    ELSE '[]'::jsonb
  END;

  v_target_locations := CASE 
    WHEN raw->'target_locations' IS NOT NULL AND jsonb_typeof(raw->'target_locations') = 'array' THEN raw->'target_locations'
    WHEN raw->'targetLocations' IS NOT NULL AND jsonb_typeof(raw->'targetLocations') = 'array' THEN raw->'targetLocations'
    ELSE '[]'::jsonb
  END;

  v_investment_size := CASE 
    WHEN raw->'investment_size' IS NOT NULL AND jsonb_typeof(raw->'investment_size') = 'array' THEN raw->'investment_size'
    WHEN raw->'investmentSize' IS NOT NULL AND jsonb_typeof(raw->'investmentSize') = 'array' THEN raw->'investmentSize'
    ELSE '[]'::jsonb
  END;

  v_industry_expertise := CASE 
    WHEN raw->'industry_expertise' IS NOT NULL AND jsonb_typeof(raw->'industry_expertise') = 'array' THEN raw->'industry_expertise'
    WHEN raw->'industryExpertise' IS NOT NULL AND jsonb_typeof(raw->'industryExpertise') = 'array' THEN raw->'industryExpertise'
    ELSE '[]'::jsonb
  END;

  v_geographic_focus := CASE 
    WHEN raw->'geographic_focus' IS NOT NULL AND jsonb_typeof(raw->'geographic_focus') = 'array' THEN raw->'geographic_focus'
    WHEN raw->'geographicFocus' IS NOT NULL AND jsonb_typeof(raw->'geographicFocus') = 'array' THEN raw->'geographicFocus'
    ELSE '[]'::jsonb
  END;

  v_equity_source := CASE 
    WHEN raw->'equity_source' IS NOT NULL AND jsonb_typeof(raw->'equity_source') = 'array' THEN raw->'equity_source'
    WHEN raw->'equitySource' IS NOT NULL AND jsonb_typeof(raw->'equitySource') = 'array' THEN raw->'equitySource'
    ELSE '[]'::jsonb
  END;

  v_financing_plan := CASE 
    WHEN raw->'financing_plan' IS NOT NULL AND jsonb_typeof(raw->'financing_plan') = 'array' THEN raw->'financing_plan'
    WHEN raw->'financingPlan' IS NOT NULL AND jsonb_typeof(raw->'financingPlan') = 'array' THEN raw->'financingPlan'
    ELSE '[]'::jsonb
  END;

  v_operating_company_targets := CASE 
    WHEN raw->'operating_company_targets' IS NOT NULL AND jsonb_typeof(raw->'operating_company_targets') = 'array' THEN raw->'operating_company_targets'
    WHEN raw->'operatingCompanyTargets' IS NOT NULL AND jsonb_typeof(raw->'operatingCompanyTargets') = 'array' THEN raw->'operatingCompanyTargets'
    ELSE '[]'::jsonb
  END;

  v_integration_plan := CASE 
    WHEN raw->'integration_plan' IS NOT NULL AND jsonb_typeof(raw->'integration_plan') = 'array' THEN raw->'integration_plan'
    WHEN raw->'integrationPlan' IS NOT NULL AND jsonb_typeof(raw->'integrationPlan') = 'array' THEN raw->'integrationPlan'
    ELSE '[]'::jsonb
  END;

  v_exclusions := CASE 
    WHEN raw->'exclusions' IS NOT NULL AND jsonb_typeof(raw->'exclusions') = 'array' THEN raw->'exclusions'
    WHEN raw->'hardExclusions' IS NOT NULL AND jsonb_typeof(raw->'hardExclusions') = 'array' THEN raw->'hardExclusions'
    ELSE '[]'::jsonb
  END;

  v_include_keywords := CASE 
    WHEN raw->'include_keywords' IS NOT NULL AND jsonb_typeof(raw->'include_keywords') = 'array' THEN raw->'include_keywords'
    WHEN raw->'includeKeywords' IS NOT NULL AND jsonb_typeof(raw->'includeKeywords') = 'array' THEN raw->'includeKeywords'
    WHEN raw->'keywords' IS NOT NULL AND jsonb_typeof(raw->'keywords') = 'array' THEN raw->'keywords'
    ELSE '[]'::jsonb
  END;
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    business_categories = CASE 
      WHEN EXCLUDED.business_categories IS NOT NULL AND jsonb_typeof(EXCLUDED.business_categories) = 'array' AND jsonb_array_length(EXCLUDED.business_categories) > 0 
      THEN EXCLUDED.business_categories 
      ELSE public.profiles.business_categories 
    END,
    target_locations = CASE 
      WHEN EXCLUDED.target_locations IS NOT NULL AND jsonb_typeof(EXCLUDED.target_locations) = 'array' AND jsonb_array_length(EXCLUDED.target_locations) > 0 
      THEN EXCLUDED.target_locations 
      ELSE public.profiles.target_locations 
    END,
    investment_size = CASE 
      WHEN EXCLUDED.investment_size IS NOT NULL AND jsonb_typeof(EXCLUDED.investment_size) = 'array' AND jsonb_array_length(EXCLUDED.investment_size) > 0 
      THEN EXCLUDED.investment_size 
      ELSE public.profiles.investment_size 
    END,
    industry_expertise = CASE 
      WHEN EXCLUDED.industry_expertise IS NOT NULL AND jsonb_typeof(EXCLUDED.industry_expertise) = 'array' AND jsonb_array_length(EXCLUDED.industry_expertise) > 0 
      THEN EXCLUDED.industry_expertise 
      ELSE public.profiles.industry_expertise 
    END,
    geographic_focus = CASE 
      WHEN EXCLUDED.geographic_focus IS NOT NULL AND jsonb_typeof(EXCLUDED.geographic_focus) = 'array' AND jsonb_array_length(EXCLUDED.geographic_focus) > 0 
      THEN EXCLUDED.geographic_focus 
      ELSE public.profiles.geographic_focus 
    END,
    equity_source = CASE 
      WHEN EXCLUDED.equity_source IS NOT NULL AND jsonb_typeof(EXCLUDED.equity_source) = 'array' AND jsonb_array_length(EXCLUDED.equity_source) > 0 
      THEN EXCLUDED.equity_source 
      ELSE public.profiles.equity_source 
    END,
    financing_plan = CASE 
      WHEN EXCLUDED.financing_plan IS NOT NULL AND jsonb_typeof(EXCLUDED.financing_plan) = 'array' AND jsonb_array_length(EXCLUDED.financing_plan) > 0 
      THEN EXCLUDED.financing_plan 
      ELSE public.profiles.financing_plan 
    END,
    operating_company_targets = CASE 
      WHEN EXCLUDED.operating_company_targets IS NOT NULL AND jsonb_typeof(EXCLUDED.operating_company_targets) = 'array' AND jsonb_array_length(EXCLUDED.operating_company_targets) > 0 
      THEN EXCLUDED.operating_company_targets 
      ELSE public.profiles.operating_company_targets 
    END,
    integration_plan = CASE 
      WHEN EXCLUDED.integration_plan IS NOT NULL AND jsonb_typeof(EXCLUDED.integration_plan) = 'array' AND jsonb_array_length(EXCLUDED.integration_plan) > 0 
      THEN EXCLUDED.integration_plan 
      ELSE public.profiles.integration_plan 
    END,
    exclusions = CASE 
      WHEN EXCLUDED.exclusions IS NOT NULL AND jsonb_typeof(EXCLUDED.exclusions) = 'array' AND jsonb_array_length(EXCLUDED.exclusions) > 0 
      THEN EXCLUDED.exclusions 
      ELSE public.profiles.exclusions 
    END,
    include_keywords = CASE 
      WHEN EXCLUDED.include_keywords IS NOT NULL AND jsonb_typeof(EXCLUDED.include_keywords) = 'array' AND jsonb_array_length(EXCLUDED.include_keywords) > 0 
      THEN EXCLUDED.include_keywords 
      ELSE public.profiles.include_keywords 
    END,
