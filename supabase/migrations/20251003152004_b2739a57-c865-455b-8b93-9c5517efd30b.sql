...
-- Fix "X active" badge to show deals per LISTING instead of per company
-- This is more reliable and meaningful in the pipeline context
...
BEGIN
  ...
  SELECT 
    ...
    -- Keep company_deal_count for backward compatibility but use better matching
    COUNT(*) OVER (PARTITION BY COALESCE(l.internal_company_name, d.contact_company, p.company)) as company_deal_count,
    -- NEW: listing_deal_count - More reliable count of deals per listing
