
-- 1) Helper: get_latest_profile_snapshot (prefers raw_signup else latest audit_old)
create or replace function public.get_latest_profile_snapshot(p_profile_id uuid)
returns table(raw_business_categories jsonb, raw_target_locations jsonb, raw_payload jsonb)
language sql
security definer
set search_path = 'public'
as $$
  select s.raw_business_categories, s.raw_target_locations, s.raw_payload
  from public.profile_data_snapshots s
  where s.profile_id = p_profile_id
  order by
    case when s.snapshot_type = 'raw_signup' then 0 else 1 end,
    s.created_at desc
  limit 1
$$;

-- 2) Preview: broaden to any latest snapshot and include industry_expertise checks
create or replace function public.preview_profile_data_restoration()
returns table(
  profile_id uuid,
  email text,
  current_categories jsonb,
  raw_categories jsonb,
  current_locations jsonb,
  raw_locations jsonb,
  restoration_needed text,
  issue_type text
)
language plpgsql
security definer
set search_path = 'public'
as $function$
declare
  p record;
  snap record;
  curr_categories jsonb;
  curr_locations jsonb;
  curr_industry jsonb;
  raw_categories jsonb;
  raw_locations jsonb;
  raw_payload jsonb;
  raw_industry jsonb;

  needs text;
  issues text;
  generic_current boolean;
  raw_industry_valid boolean;
begin
  for p in
    select id, email, business_categories, target_locations, industry_expertise
    from public.profiles
  loop
    -- Get latest available snapshot (raw_signup preferred, else audit_old)
    select s.raw_business_categories, s.raw_target_locations, s.raw_payload
    into raw_categories, raw_locations, raw_payload
    from public.get_latest_profile_snapshot(p.id) s;

    if raw_categories is null and raw_locations is null and raw_payload is null then
      continue; -- no snapshot available for this profile
    end if;

    curr_categories := p.business_categories;
    curr_locations := p.target_locations;
    curr_industry := p.industry_expertise;
    raw_industry := nullif(coalesce(raw_payload->'industry_expertise', raw_payload->'industryExpertise'), 'null'::jsonb);

    needs := '';
    issues := '';

    -- Category issues (same as before)
    if raw_categories is not null and curr_categories is not null then
      if jsonb_array_length(curr_categories) <= 2 and jsonb_array_length(raw_categories) >= 3 then
        needs := needs || 'Categories: Over-standardized; ';
        issues := case when issues = '' then 'over_standardized' else issues || ',over_standardized' end;
      elsif (curr_categories ? 'Technology & Software') and jsonb_array_length(curr_categories) = 1
            and not (raw_categories ? 'Technology & Software' or raw_categories ? 'technology' or raw_categories ? 'software') then
        needs := needs || 'Categories: Wrong standardization; ';
        issues := case when issues = '' then 'wrong_standardization' else issues || ',wrong_standardization' end;
      elsif ((raw_categories ? 'All Industries') or (raw_categories ? 'any') or (raw_categories ? 'all'))
            and not (curr_categories ? 'All Industries') then
        needs := needs || 'Categories: Lost "All Industries"; ';
        issues := case when issues = '' then 'lost_all_industries' else issues || ',lost_all_industries' end;
      end if;
    end if;

    -- Location issues (same as before)
    if raw_locations is not null and curr_locations is not null then
      if (raw_locations ? 'Anywhere' or raw_locations ? 'anywhere' or raw_locations ? 'lower 48' or raw_locations ? 'nationwide')
         and not (curr_locations ? 'United States') then
        needs := needs || 'Locations: Lost specific descriptions; ';
        issues := case when issues = '' then 'lost_location_descriptions' else issues || ',lost_location_descriptions' end;
      end if;
    end if;

    -- Industry expertise issues (new)
    raw_industry_valid := false;

    if raw_industry is not null then
      if jsonb_typeof(raw_industry) = 'array' then
        -- valid if ANY element is non-garbage (length > 2, not only f's, not n/a/test)
        select exists (
          select 1
          from jsonb_array_elements_text(raw_industry) as e(val)
          where length(trim(e.val)) > 2
            and lower(e.val) not in ('n/a','na','test','ffffffffffff')
            and e.val !~* '^[f]+$'
        ) into raw_industry_valid;
      else
        -- treat as string
        raw_industry_valid := (
          length(trim(raw_industry->>'')) > 2
          and lower(raw_industry->>'') not in ('n/a','na','test','ffffffffffff')
          and (raw_industry->>'' !~ '^[f]+$')
        );
      end if;
    end if;

    -- Generic current check (specifically the overwritten combo)
    generic_current := (curr_industry is not null
                        and jsonb_typeof(curr_industry) = 'array'
                        and jsonb_array_length(curr_industry) = 2
                        and curr_industry @> '["Technology","Manufacturing"]'::jsonb);

    -- If generic overwrite or corrupt Fs detected
    if generic_current then
      if raw_industry_valid then
        needs := needs || 'Industry Expertise: Overwritten with generic, should restore from raw; ';
        issues := case when issues = '' then 'industry_overwrite' else issues || ',industry_overwrite' end;
      else
        needs := needs || 'Industry Expertise: Overwritten with generic and raw is invalid -> clear; ';
        issues := case when issues = '' then 'industry_corrupted' else issues || ',industry_corrupted' end;
      end if;
    elsif curr_industry is not null and (
      (jsonb_typeof(curr_industry) = 'string' and lower(curr_industry->>'') in ('ffffffffffff','n/a','na','test'))
      or (jsonb_typeof(curr_industry) = 'string' and (curr_industry->>'' ~ '^[fF]+$'))
      or (jsonb_typeof(curr_industry) = 'array' and not exists (
          select 1 from jsonb_array_elements_text(curr_industry) e(val)
          where length(trim(e.val)) > 2 and e.val !~* '^[f]+$' and lower(e.val) not in ('n/a','na','test','ffffffffffff')
      ))
    ) then
      needs := needs || 'Industry Expertise: Corrupted (garbage/test) -> clear; ';
      issues := case when issues = '' then 'industry_corrupted' else issues || ',industry_corrupted' end;
    end if;

    if needs <> '' then
      profile_id := p.id;
      email := p.email;
      current_categories := curr_categories;
      raw_categories := raw_categories;
      current_locations := curr_locations;
      raw_locations := raw_locations;
      restoration_needed := needs;
      issue_type := issues;
      return next;
    end if;
  end loop;

  return;
end;
$function$;

-- 3) Restore: broaden to any latest snapshot and add industry_expertise repair
create or replace function public.restore_profile_data_automated()
returns table(profile_id uuid, restoration_type text, old_value jsonb, new_value jsonb, details text)
language plpgsql
security definer
set search_path = 'public'
as $function$
declare
  p record;
  curr_categories jsonb;
  curr_locations jsonb;
  curr_industry jsonb;

  snap_raw_categories jsonb;
  snap_raw_locations jsonb;
  snap_raw_payload jsonb;
  snap_raw_industry jsonb;

  should_restore_categories boolean;
  should_restore_locations boolean;

  generic_current boolean;
  raw_industry_valid boolean;
  restored_count integer := 0;
begin
  for p in
    select id, email, business_categories, target_locations, industry_expertise
    from public.profiles
  loop
    -- Load best snapshot
    select s.raw_business_categories, s.raw_target_locations, s.raw_payload
    into snap_raw_categories, snap_raw_locations, snap_raw_payload
    from public.get_latest_profile_snapshot(p.id) s;

    if snap_raw_categories is null and snap_raw_locations is null and snap_raw_payload is null then
      continue;
    end if;

    curr_categories := p.business_categories;
    curr_locations := p.target_locations;
    curr_industry := p.industry_expertise;
    snap_raw_industry := nullif(coalesce(snap_raw_payload->'industry_expertise', snap_raw_payload->'industryExpertise'), 'null'::jsonb);

    should_restore_categories := false;
    should_restore_locations := false;

    -- Categories restore (same rules as before)
    if snap_raw_categories is not null and curr_categories is not null then
      if jsonb_array_length(curr_categories) <= 2 and jsonb_array_length(snap_raw_categories) >= 3 then
        should_restore_categories := true;
      end if;

      if (curr_categories ? 'Technology & Software') and
         jsonb_array_length(curr_categories) = 1 and
         not (snap_raw_categories ? 'Technology & Software' or snap_raw_categories ? 'technology' or snap_raw_categories ? 'software') then
        should_restore_categories := true;
      end if;

      if (snap_raw_categories ? 'All Industries' or snap_raw_categories ? 'any' or snap_raw_categories ? 'all')
         and not (curr_categories ? 'All Industries') then
        should_restore_categories := true;
      end if;
    end if;

    -- Locations restore (same rules as before)
    if snap_raw_locations is not null and curr_locations is not null then
      if (snap_raw_locations ? 'Anywhere' or snap_raw_locations ? 'anywhere' or snap_raw_locations ? 'lower 48' or snap_raw_locations ? 'nationwide')
         and not (curr_locations ? 'United States') then
        should_restore_locations := true;
      end if;
    end if;

    -- Apply categories
    if should_restore_categories then
      update public.profiles
      set business_categories = snap_raw_categories,
          updated_at = now()
      where id = p.id;

      profile_id := p.id;
      restoration_type := 'business_categories';
      old_value := curr_categories;
      new_value := snap_raw_categories;
      details := 'Restored over-standardized categories from latest snapshot';
      restored_count := restored_count + 1;
      return next;
    end if;

    -- Apply locations
    if should_restore_locations then
      update public.profiles
      set target_locations = snap_raw_locations,
          updated_at = now()
      where id = p.id;

      profile_id := p.id;
      restoration_type := 'target_locations';
      old_value := curr_locations;
      new_value := snap_raw_locations;
      details := 'Restored over-standardized locations from latest snapshot';
      restored_count := restored_count + 1;
      return next;
    end if;

    -- Industry expertise restoration/cleanup (new)
    -- Determine validity of snapshot raw industry
    raw_industry_valid := false;
    if snap_raw_industry is not null then
      if jsonb_typeof(snap_raw_industry) = 'array' then
        select exists (
          select 1
          from jsonb_array_elements_text(snap_raw_industry) as e(val)
          where length(trim(e.val)) > 2
            and lower(e.val) not in ('n/a','na','test','ffffffffffff')
            and e.val !~* '^[f]+$'
        ) into raw_industry_valid;
      else
        raw_industry_valid := (
          length(trim(snap_raw_industry->>'')) > 2
          and lower(snap_raw_industry->>'') not in ('n/a','na','test','ffffffffffff')
          and (snap_raw_industry->>'' !~ '^[f]+$')
        );
      end if;
    end if;

    generic_current := (curr_industry is not null
                        and jsonb_typeof(curr_industry) = 'array'
                        and jsonb_array_length(curr_industry) = 2
                        and curr_industry @> '["Technology","Manufacturing"]'::jsonb);

    -- Case A: Current is generic overwrite
    if generic_current then
      if raw_industry_valid then
        update public.profiles
        set industry_expertise = snap_raw_industry,
            updated_at = now()
        where id = p.id;

        profile_id := p.id;
        restoration_type := 'industry_expertise';
        old_value := curr_industry;
        new_value := snap_raw_industry;
        details := 'Replaced generic overwrite with valid raw industry_expertise from snapshot';
        restored_count := restored_count + 1;
        return next;
      else
        update public.profiles
        set industry_expertise = null,
            updated_at = now()
        where id = p.id;

        profile_id := p.id;
        restoration_type := 'industry_expertise';
        old_value := curr_industry;
        new_value := to_jsonb(null);
        details := 'Cleared generic overwrite; snapshot value invalid/garbage';
        restored_count := restored_count + 1;
        return next;
      end if;
    end if;

    -- Case B: Current itself is corrupted (garbage Fs, NA/test)
    if curr_industry is not null and (
      (jsonb_typeof(curr_industry) = 'string' and lower(curr_industry->>'') in ('ffffffffffff','n/a','na','test'))
      or (jsonb_typeof(curr_industry) = 'string' and (curr_industry->>'' ~ '^[fF]+$'))
      or (jsonb_typeof(curr_industry) = 'array' and not exists (
          select 1 from jsonb_array_elements_text(curr_industry) e(val)
          where length(trim(e.val)) > 2 and e.val !~* '^[f]+$' and lower(e.val) not in ('n/a','na','test','ffffffffffff')
      ))
    ) then
      update public.profiles
      set industry_expertise = null,
          updated_at = now()
      where id = p.id;

      profile_id := p.id;
      restoration_type := 'industry_expertise';
      old_value := curr_industry;
      new_value := to_jsonb(null);
      details := 'Cleared corrupted industry_expertise';
      restored_count := restored_count + 1;
      return next;
    end if;

  end loop;

  -- Summarize in audit logs
  insert into public.audit_logs (table_name, operation, metadata, admin_id)
  values (
    'profiles',
    'automated_data_restoration',
    jsonb_build_object('total_restorations', restored_count, 'timestamp', now()),
    auth.uid()
  );

  return;
end;
$function$;
