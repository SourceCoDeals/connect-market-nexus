-- Update notify_deal_reassignment trigger to send emails to new owner
-- This will invoke the edge function to notify the new owner when assigned
...
begin
  ...
  if new.assigned_to is distinct from old.assigned_to then
    ...
    if new.assigned_to is not null then
      ...
      -- Invoke edge function to send email to new owner
      perform net.http_post(
        url := (select current_setting('app.settings.supabase_url', true) || '/functions/v1/notify-new-deal-owner'),
        ...
        body := jsonb_build_object(
          ...
          'buyerCompany', v_buyer_company,
          'assignedByName', v_modifying_admin_name
        )
      );
    end if;
  end if;
