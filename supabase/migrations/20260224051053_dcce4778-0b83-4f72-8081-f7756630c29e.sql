...
-- Migration: update_deal_creation_triggers
-- Updates get_deals_with_details RPC to include new FK columns
...
RETURNS TABLE(
  ...
  deal_followed_up_at timestamptz,
  deal_followed_up_by uuid,
  deal_negative_followed_up boolean,
  deal_negative_followed_up_at timestamptz,
...
BEGIN
  ...
  SELECT
    ...
    (SELECT MAX(dc.created_at) FROM deal_contacts dc WHERE dc.deal_id = d.id) as last_contact_at,
    (SELECT COUNT(*)::integer FROM deal_activities da WHERE da.deal_id = d.id) as total_activities,
    (SELECT COUNT(*)::integer FROM deal_tasks dt WHERE dt.deal_id = d.id AND dt.status = 'pending') as pending_tasks,
    (SELECT COUNT(*)::integer FROM deal_tasks dt WHERE dt.deal_id = d.id) as total_tasks,
    (SELECT COUNT(*)::integer FROM deal_tasks dt WHERE dt.deal_id = d.id AND dt.status = 'completed') as completed_tasks,
    GREATEST(
      d.updated_at,
    ...
    COALESCE(bc.first_name || ' ' || bc.last_name, '') as buyer_contact_name,
    COALESCE(sc.first_name || ' ' || sc.last_name, '') as seller_contact_name,
    rb.company_name as remarketing_buyer_name
  FROM deals d
  ...
  LEFT JOIN remarketing_buyers rb ON d.remarketing_buyer_id = rb.id
