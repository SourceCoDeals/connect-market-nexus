-- Comprehensive data standardization and backfill migration
...
-- Step 4: Ensure target_deal_size fields are numeric for independent sponsors
UPDATE public.profiles SET
  target_deal_size_min = NULL
WHERE buyer_type = 'independentSponsor' 
  AND target_deal_size_min IS NOT NULL 
  AND target_deal_size_min !~ '^\d+(\.\d+)?$';

UPDATE public.profiles SET
  target_deal_size_max = NULL
WHERE buyer_type = 'independentSponsor' 
  AND target_deal_size_max IS NOT NULL 
  AND target_deal_size_max !~ '^\d+(\.\d+)?$';

-- Step 5: Fix the specific adambhaile00@gmail.com profile
UPDATE public.profiles SET
