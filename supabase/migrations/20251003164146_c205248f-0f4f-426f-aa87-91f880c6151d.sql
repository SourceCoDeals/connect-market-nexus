-- Phase 1: Add buyer_connection_count to get_deals_with_details
-- This counts all connection requests by the same buyer (not deals on the same listing)
...
BEGIN
  ...
  SELECT 
    ...
    -- Keep company_deal_count for backward compatibility
    COUNT(*) OVER (PARTITION BY COALESCE(l.internal_company_name, d.contact_company, p.company)) as company_deal_count,
    -- listing_deal_count - Count of deals per listing
    COUNT(*) OVER (PARTITION BY d.listing_id) as listing_deal_count,
    -- NEW: buyer_connection_count - Count of ALL connection requests by this buyer
    (
      SELECT COUNT(*)::bigint
