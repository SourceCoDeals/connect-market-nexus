-- ============================================================================
-- MIGRATION: contacts_upsert UPDATE branch — allow clearing structured phones
-- ============================================================================
-- Before this migration, contacts_upsert()'s UPDATE branch used
--
--   mobile_phone_1 = COALESCE(NULLIF(trim(p_fields->>'mobile_phone_1'), ''),
--                             mobile_phone_1),
--
-- which treats both "key missing" and "key present with empty string" as
-- "keep existing value." Combined with UI mutations that normalize a cleared
-- input to null/'' before calling the RPC, there was no way for a caller to
-- intentionally clear a structured phone once it had been set.
--
-- Switch to the CASE-WHEN idiom already used by update_buyer_contact():
--
--   * Missing key         → keep existing (p_fields->>'x' returns NULL)
--   * Key with JSON null  → keep existing (same NULL result, preserves
--                           back-compat with callers that pass null to mean
--                           "don't touch")
--   * Key with ''         → clear (NULLIF(TRIM(''), '') → NULL)
--   * Key with "   "      → clear (trimmed to empty → NULLIF → NULL)
--   * Key with "555…"     → set (trimmed value)
--
-- Only the five phone-related columns are changed; every other column keeps
-- its original COALESCE semantics because no bug has been reported against
-- them and widening the behavior change without a reason would risk breaking
-- CSV importers and webhook callers that lean on "omit to keep."
-- ============================================================================
...
BEGIN
  ...
  ELSE
    ...
    -- provide a new one. Structured phone columns use CASE-WHEN so callers
    -- can explicitly clear them (see migration header).
    ...
    SET
      ...
      mobile_phone_1 = CASE
        WHEN p_fields->>'mobile_phone_1' IS NULL THEN mobile_phone_1
        ELSE NULLIF(trim(p_fields->>'mobile_phone_1'), '')
      END,
      mobile_phone_2 = CASE
        WHEN p_fields->>'mobile_phone_2' IS NULL THEN mobile_phone_2
        ELSE NULLIF(trim(p_fields->>'mobile_phone_2'), '')
      END,
      mobile_phone_3 = CASE
        WHEN p_fields->>'mobile_phone_3' IS NULL THEN mobile_phone_3
        ELSE NULLIF(trim(p_fields->>'mobile_phone_3'), '')
      END,
      office_phone   = CASE
        WHEN p_fields->>'office_phone' IS NULL THEN office_phone
        ELSE NULLIF(trim(p_fields->>'office_phone'), '')
      END,
      phone_source   = CASE
        WHEN p_fields->>'phone_source' IS NULL THEN phone_source
        ELSE NULLIF(trim(p_fields->>'phone_source'), '')
      END,
      confidence   = CASE
...
COMMENT ON FUNCTION public.contacts_upsert(JSONB, JSONB, TEXT, JSONB) IS
  ...
  'history row to contact_events atomically. Structured phone fields '
  '(mobile_phone_1/2/3, office_phone, phone_source) accept explicit empty '
  'strings to clear; JSON null or a missing key keeps the existing value.';
