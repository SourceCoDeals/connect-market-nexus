...
-- Fix: update_fee_agreement_firm_status (text overload)
CREATE OR REPLACE FUNCTION public.update_fee_agreement_firm_status(p_firm_id uuid, p_value boolean, p_signer_id uuid DEFAULT NULL::uuid, p_admin_notes text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- Fix: update_fee_agreement_firm_status (timestamptz overload)
CREATE OR REPLACE FUNCTION public.update_fee_agreement_firm_status(p_firm_id uuid, p_value boolean, p_signer_id uuid, p_signed_at timestamp with time zone)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- Fix: update_lead_fee_agreement_status
