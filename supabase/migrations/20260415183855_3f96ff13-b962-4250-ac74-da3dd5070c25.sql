...
BEGIN
  ...
  v_first_external_referrer := v_raw_meta->>'first_external_referrer';
  v_first_blog_landing := v_raw_meta->>'first_blog_landing';
  v_first_utm_source := v_raw_meta->>'first_utm_source';
  
  -- Parse first_seen_at timestamp
  BEGIN
    v_first_seen_at := (v_raw_meta->>'first_seen_at')::timestamptz;
  EXCEPTION WHEN OTHERS THEN
    v_first_seen_at := NULL;
  END;

  -- If visitor_id is available, try to enrich attribution from user_journeys
  IF v_visitor_id IS NOT NULL AND v_visitor_id != '' THEN
    BEGIN
      SELECT
        COALESCE(v_first_external_referrer, uj.first_referrer),
        COALESCE(v_first_blog_landing, uj.first_landing_page),
        COALESCE(v_first_seen_at, uj.first_seen_at),
        COALESCE(v_first_utm_source, uj.first_utm_source)
      INTO
      ...
      LIMIT 1;
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'handle_new_user: user_journeys lookup failed: %', SQLERRM;
    END;

    -- Fallback: try user_sessions if journey lookup found nothing
    IF v_first_external_referrer IS NULL AND v_first_utm_source IS NULL THEN
      BEGIN
        SELECT
          us.referrer,
          us.landing_page,
        ...
        LIMIT 1;
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'handle_new_user: user_sessions lookup failed: %', SQLERRM;
      END;
    END IF;
  END IF;

  -- Attempt the full profile upsert
  BEGIN
    INSERT INTO public.profiles (
      id, email, first_name, last_name, company, 
      buyer_type, phone_number, linkedin_profile, website,
      role,
      business_categories, target_locations,
      revenue_range_min, revenue_range_max,
      ideal_target_description, specific_business_search,
      referral_source, referral_source_detail,
      job_title,
      -- Search Fund fields
      search_type, acq_equity_band, financing_plan,
      flex_sub2m_ebitda, anchor_investors_summary, search_stage,
      -- Corporate fields
      estimated_revenue, owning_business_unit, deal_size_band,
      integration_plan, corpdev_intent,
      -- PE fields
      fund_size, investment_size, aum,
      portfolio_company_addon, deploying_capital_now,
      -- Family Office fields
      discretion_type, permanent_capital, operating_company_targets,
      -- Independent Sponsor fields
      committed_equity_band, equity_source, flex_subxm_ebitda,
      backers_summary, deployment_timing,
      -- Individual fields
      funding_source, needs_loan,
      max_equity_today_band, uses_bank_finance,
      -- Advisor fields
      on_behalf_of_buyer, buyer_role, buyer_org_url, mandate_blurb,
      -- Business Owner fields
      owner_intent, owner_timeline,
      -- New Step 4 fields
      deal_intent, exclusions, include_keywords,
      -- Deal sourcing fields
      deal_sourcing_methods, target_acquisition_volume,
      -- Attribution fields (NO visitor_id -- column does not exist on profiles)
      first_external_referrer, first_blog_landing,
      first_seen_at, first_utm_source,
      created_at, updated_at
    )
    VALUES (
      NEW.id, NEW.email, v_first_name, v_last_name, v_company,
      v_buyer_type, v_phone, v_linkedin, v_website,
      'buyer',
      ARRAY(SELECT jsonb_array_elements_text(v_categories)),
      ARRAY(SELECT jsonb_array_elements_text(v_locations)),
      v_revenue_min, v_revenue_max,
      v_ideal_target, v_specific_search,
      v_referral_source, v_referral_detail,
      v_job_title,
      v_search_type, v_acq_equity_band,
      ARRAY(SELECT jsonb_array_elements_text(v_financing_plan)),
      v_flex_sub2m, v_anchor_investors, v_search_stage,
      v_estimated_revenue, v_owning_business_unit, v_deal_size_band,
      ARRAY(SELECT jsonb_array_elements_text(v_integration_plan)),
      v_corpdev_intent,
      v_fund_size,
      ARRAY(SELECT jsonb_array_elements_text(v_investment_size)),
      v_aum,
      v_portfolio_addon, v_deploying_capital,
      v_discretion_type, v_permanent_capital,
      ARRAY(SELECT jsonb_array_elements_text(v_operating_targets)),
      v_committed_equity_band,
      ARRAY(SELECT jsonb_array_elements_text(v_equity_source)),
      v_flex_subxm,
      v_backers_summary, v_deployment_timing,
      v_funding_source, v_needs_loan,
      v_max_equity_today, v_uses_bank_finance,
      v_on_behalf_of, v_buyer_role, v_buyer_org_url, v_mandate_blurb,
      v_owner_intent, v_owner_timeline,
      v_deal_intent,
      ARRAY(SELECT jsonb_array_elements_text(v_exclusions)),
      ARRAY(SELECT jsonb_array_elements_text(v_include_keywords)),
      ARRAY(SELECT jsonb_array_elements_text(v_deal_sourcing_methods)),
      v_target_acquisition_volume,
      v_first_external_referrer, v_first_blog_landing,
      v_first_seen_at, v_first_utm_source,
      NOW(), NOW()
    )
    ON CONFLICT (id) DO UPDATE SET
      ...
      linkedin_profile = COALESCE(EXCLUDED.linkedin_profile, profiles.linkedin_profile),
      ...
      search_type = COALESCE(EXCLUDED.search_type, profiles.search_type),
      acq_equity_band = COALESCE(EXCLUDED.acq_equity_band, profiles.acq_equity_band),
      financing_plan = COALESCE(EXCLUDED.financing_plan, profiles.financing_plan),
      flex_sub2m_ebitda = COALESCE(EXCLUDED.flex_sub2m_ebitda, profiles.flex_sub2m_ebitda),
      anchor_investors_summary = COALESCE(EXCLUDED.anchor_investors_summary, profiles.anchor_investors_summary),
      search_stage = COALESCE(EXCLUDED.search_stage, profiles.search_stage),
      estimated_revenue = COALESCE(EXCLUDED.estimated_revenue, profiles.estimated_revenue),
      owning_business_unit = COALESCE(EXCLUDED.owning_business_unit, profiles.owning_business_unit),
      deal_size_band = COALESCE(EXCLUDED.deal_size_band, profiles.deal_size_band),
      integration_plan = COALESCE(EXCLUDED.integration_plan, profiles.integration_plan),
      corpdev_intent = COALESCE(EXCLUDED.corpdev_intent, profiles.corpdev_intent),
      fund_size = COALESCE(EXCLUDED.fund_size, profiles.fund_size),
      investment_size = COALESCE(EXCLUDED.investment_size, profiles.investment_size),
      aum = COALESCE(EXCLUDED.aum, profiles.aum),
      portfolio_company_addon = COALESCE(EXCLUDED.portfolio_company_addon, profiles.portfolio_company_addon),
      deploying_capital_now = COALESCE(EXCLUDED.deploying_capital_now, profiles.deploying_capital_now),
      discretion_type = COALESCE(EXCLUDED.discretion_type, profiles.discretion_type),
      permanent_capital = COALESCE(EXCLUDED.permanent_capital, profiles.permanent_capital),
      operating_company_targets = COALESCE(EXCLUDED.operating_company_targets, profiles.operating_company_targets),
      committed_equity_band = COALESCE(EXCLUDED.committed_equity_band, profiles.committed_equity_band),
      equity_source = COALESCE(EXCLUDED.equity_source, profiles.equity_source),
      flex_subxm_ebitda = COALESCE(EXCLUDED.flex_subxm_ebitda, profiles.flex_subxm_ebitda),
      backers_summary = COALESCE(EXCLUDED.backers_summary, profiles.backers_summary),
      deployment_timing = COALESCE(EXCLUDED.deployment_timing, profiles.deployment_timing),
      funding_source = COALESCE(EXCLUDED.funding_source, profiles.funding_source),
      needs_loan = COALESCE(EXCLUDED.needs_loan, profiles.needs_loan),
      max_equity_today_band = COALESCE(EXCLUDED.max_equity_today_band, profiles.max_equity_today_band),
      uses_bank_finance = COALESCE(EXCLUDED.uses_bank_finance, profiles.uses_bank_finance),
      on_behalf_of_buyer = COALESCE(EXCLUDED.on_behalf_of_buyer, profiles.on_behalf_of_buyer),
      buyer_role = COALESCE(EXCLUDED.buyer_role, profiles.buyer_role),
      buyer_org_url = COALESCE(EXCLUDED.buyer_org_url, profiles.buyer_org_url),
      mandate_blurb = COALESCE(EXCLUDED.mandate_blurb, profiles.mandate_blurb),
      owner_intent = COALESCE(EXCLUDED.owner_intent, profiles.owner_intent),
      owner_timeline = COALESCE(EXCLUDED.owner_timeline, profiles.owner_timeline),
      ...
      updated_at = NOW();
  EXCEPTION WHEN OTHERS THEN
    -- Full INSERT failed -- create a minimal profile so signup never crashes
    RAISE NOTICE 'handle_new_user: full profile insert failed: % (%). Attempting minimal fallback.', SQLERRM, SQLSTATE;
    BEGIN
      INSERT INTO public.profiles (id, email, first_name, last_name, role, created_at, updated_at)
      VALUES (NEW.id, NEW.email, COALESCE(v_first_name, ''), COALESCE(v_last_name, ''), 'buyer', NOW(), NOW())
      ON CONFLICT (id) DO NOTHING;
    EXCEPTION WHEN OTHERS THEN
      -- Even minimal insert failed -- log and re-raise as last resort
      RAISE NOTICE 'handle_new_user: minimal fallback also failed: % (%). Signup will fail.', SQLERRM, SQLSTATE;
      RAISE;
    END;
  END;

  -- Log successful trigger execution (non-blocking)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'handle_new_user: success log insert failed: %', SQLERRM;
  END;
...
EXCEPTION WHEN OTHERS THEN
  ...
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'handle_new_user: failure log insert also failed: %', SQLERRM;
  END;
