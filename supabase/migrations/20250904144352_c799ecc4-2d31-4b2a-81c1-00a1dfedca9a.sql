-- Drop existing function to recreate it with proper task counting
DROP FUNCTION IF EXISTS public.get_deals_with_details();

-- Recreate the function with correct task counting logic
...
BEGIN
  ...
  SELECT 
    ...
    d.assigned_to,
    assigned_admin.first_name || ' ' || assigned_admin.last_name as assigned_admin_name,
