...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_buyer_introductions_workspace ON buyer_introductions(workspace_id);
...
-- Policies
DROP POLICY IF EXISTS buyer_introductions_workspace_policy ON buyer_introductions;
CREATE POLICY buyer_introductions_workspace_policy ON buyer_introductions
  FOR ALL
  USING (auth.jwt() ->> 'workspace_id' = workspace_id::text);

DROP POLICY IF EXISTS introduction_status_log_policy ON introduction_status_log;
CREATE POLICY introduction_status_log_policy ON introduction_status_log
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM buyer_introductions
      WHERE id = buyer_introduction_id
      AND workspace_id = (auth.jwt() ->> 'workspace_id')::uuid
    )
  );

DROP POLICY IF EXISTS introduction_activity_policy ON introduction_activity;
CREATE POLICY introduction_activity_policy ON introduction_activity
  FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM buyer_introductions
...
SELECT
  bi.id, bi.workspace_id, bi.buyer_name, bi.buyer_firm_name,
  ...
  bi.internal_champion, bi.created_at,
...
SELECT
  ...
  bi.next_step, bi.expected_next_step_date,
  CASE
